...
void MonLogFrameDlg::OnDelAllFile(void)
{
    ...
    */
    ...
    msgBox.SetMessLabelText("是否要清除所有日志?清除后会自动重启系统!");
    ...
    if(QMessageBox::Accepted == msgBox.exec())
    {
        ...
        g_pAlmFile->Delete();
        strPathName  = qApp->applicationDirPath()+"/";
        strPathName += g_devConfigInfo.basicCfgInfo.strLogPath;
        strPathName += ALM_FILE_NAME;
        g_pAlmFile->CreateFile(strPathName);
...
/*
void MonLogFrameDlg::OnDelCurrFile(void)//删除当前
{
    QString strPathName,strDelCmd;
    QPushButton* pNewBut;
    QMessageBox  msgBox(this);
...
{/**/
	...
	ReadOperLogData((char*)&m_readLogData, 100, m_unLogMovFlag);
	ShowOperLogData();
	if(m_unLogNum < g_operLogInfo.unOperLogFileNum)
	{
    	m_unLogNum++;
	}
	ShowSite(m_unLogNum);
...
{/**/
    ...
    ui->label->setText(QString(strPar+strNum));
}
...
void MonLogFrameDlg::CreateAlmLogWidget(void)
{
	...
	m_wAlmWidget[0]->setGeometry(QRect(0, 0, 537, 460));
	m_wAlmWidget[0]->setStyleSheet("background: transparent;");
	...
	m_wAlmWidget[1]->setGeometry(QRect(0, 0, 537, 460));
	m_wAlmWidget[1]->setStyleSheet("background: transparent;");
    QHBoxLayout* pFirstRow  = new QHBoxLayout();
    ...
    for(UINT8 i = 0; i < 2; i++)
    {
		...
		m_pAlmUTmpA[i] = new QLineEdit(m_wAlmWidget[i]);
		m_pAlmUTmpA[i]->setReadOnly(true);
		m_pAlmUTmpA[i]->setMaximumWidth(lEditWidth);

		pFirstRow  = new QHBoxLayout();
		AddRowItem(pFirstRow,m_pAlmUPresA[i],tr("压力上限:"));
		AddRowItem(pFirstRow,m_pAlmUTmpA[i],tr("血温上限:"));
		...
		m_pAlmBubblA[i] = new QLineEdit(m_wAlmWidget[i]);
		m_pAlmBubblA[i]->setReadOnly(true);
		m_pAlmBubblA[i]->setMaximumWidth(lEditWidth);
		...
		AddRowItem(pFirstRow,m_pAlmBubblA[i],tr("汽包告警:"));
		...
		m_pAlmUTmpV[i] = new QLineEdit(m_wAlmWidget[i]);
		m_pAlmUTmpV[i]->setReadOnly(true);
		m_pAlmUTmpV[i]->setMaximumWidth(lEditWidth);

		pFirstRow  = new QHBoxLayout();
		AddRowItem(pFirstRow,m_pAlmUPresV[i],tr("压力上限:"));
		AddRowItem(pFirstRow,m_pAlmUTmpV[i],tr("血温上限:"));
		formLayout[i]->addRow(pFirstRow);
		...
		m_pAlmBubblV[i] = new QLineEdit(m_wAlmWidget[i]);
		m_pAlmBubblV[i]->setReadOnly(true);
		m_pAlmBubblV[i]->setMaximumWidth(lEditWidth);		
		pFirstRow  = new QHBoxLayout();
		AddRowItem(pFirstRow,m_pAlmUFlowV[i],tr("流量上限:"));
		AddRowItem(pFirstRow,m_pAlmLFlowV[i],tr("流量下限:"));
		AddRowItem(pFirstRow,m_pAlmBubblV[i],tr("汽包告警:"));
		formLayout[i]->addRow(pFirstRow);
...
void MonLogFrameDlg::CreatePourLogWidget(void)
{
	...
	m_wPourWidget[0]->setGeometry(QRect(0, 0, 537, 460));
	m_wPourWidget[0]->setStyleSheet("background: transparent;");
	...
	m_wPourWidget[1]->setGeometry(QRect(0, 0, 537, 460));
	m_wPourWidget[1]->setStyleSheet("background: transparent;");
    ...
    for(UINT8 i = 0; i < 2; i++)
    {
		...
		m_pPourUTmpA[i] = new QLineEdit(m_wPourWidget[i]);
		m_pPourUTmpA[i]->setReadOnly(true);
		m_pPourUTmpA[i]->setMaximumWidth(lEditWidth);		
		formLayout[i]->addRow(tr("血温上限:"), m_pPourUTmpA[i]);
		...
		m_pPourUTmpV[i] = new QLineEdit(m_wPourWidget[i]);
		m_pPourUTmpV[i]->setReadOnly(true);
		m_pPourUTmpV[i]->setMaximumWidth(lEditWidth);
		formLayout[i]->addRow(tr("血温上限:"), m_pPourUTmpV[i]);
...
void MonLogFrameDlg::CreateWatLogWidget(void)
{
	...
	m_wWatWidget[0]->setGeometry(QRect(0, 0, 537, 460));
	m_wWatWidget[0]->setStyleSheet("background: transparent;");
	...
	m_wWatWidget[1]->setGeometry(QRect(0, 0, 537, 460));
	m_wWatWidget[1]->setStyleSheet("background: transparent;");
    ...
    for(int i = 0; i < 2; i++)
    {
        ...
        pLabel[i] = new QLabel(m_wWatWidget[i]);//
        pLabel[i]->setMinimumSize(300,30);
        pLabel[i]->setText(tr("----水浴设置值上限配置----"));
        formLayout[i]->addRow(tr(""),pLabel[i]);	
		
		m_pWatTmpValLimt[i] = new QLineEdit(m_wWatWidget[i]);
		m_pWatTmpValLimt[i]->setReadOnly(true);
		m_pWatTmpValLimt[i]->setMaximumWidth(lEditWidth);
		formLayout[i]->addRow(tr("温度上限:"), m_pWatTmpValLimt[i]);		
...
void MonLogFrameDlg::CreatePumLogWidget(void)
{
	...
	m_wPumWidget[0]->setGeometry(QRect(0, 0, 537, 460));
	m_wPumWidget[0]->setStyleSheet("background: transparent;");
	...
	m_wPumWidget[1]->setGeometry(QRect(0, 0, 537, 460));
	m_wPumWidget[1]->setStyleSheet("background: transparent;");
    ...
    for(int i = 0; i < 2; i++)
    {
        ...
        pLabel[i] = new QLabel(m_wPumWidget[i]);//
        pLabel[i]->setMinimumSize(300,30);
        pLabel[i]->setText(tr("----血泵设置值上限配置----"));
        formLayout[i]->addRow(tr(""),pLabel[i]);		

		m_pPumPreeValLimt[i] = new QLineEdit(m_wPumWidget[i]);
		m_pPumPreeValLimt[i]->setReadOnly(true);
		m_pPumPreeValLimt[i]->setMaximumWidth(lEditWidth);
		formLayout[i]->addRow(tr("压力上限:"), m_pPumPreeValLimt[i]);

		m_pPumSpeeValLimt[i] = new QLineEdit(m_wPumWidget[i]);
		m_pPumSpeeValLimt[i]->setReadOnly(true);
		m_pPumSpeeValLimt[i]->setMaximumWidth(lEditWidth);
		formLayout[i]->addRow(tr("转速上限:"), m_pPumSpeeValLimt[i]);
...
void MonLogFrameDlg::CreateCtlLogWidg(void)
{
	...
	m_wCtlWidget[0]->setGeometry(QRect(0, 0, 537, 460));
	m_wCtlWidget[0]->setStyleSheet("background: transparent;");
	...
	m_wCtlWidget[1]->setGeometry(QRect(0, 0, 537, 460));
	m_wCtlWidget[1]->setStyleSheet("background: transparent;");
...
QString MonLogFrameDlg::GetStite(UINT8 stite)
{
	...
	else if(stite == SYS_CTL)
	{
		reString = "系统";
	}
	else if(stite == SWT_CTL)
	{
		reString = "灌注切换";
	}
	else if(stite == WAT_TMP)
	{
		reString = "水过温";
	}
	else if(stite == APUM_TMP)
	{
		reString = "A血过温";
	}
	else if(stite == VPUM_TMP)
	{
		reString = "V血过温";
	}
...
QString MonLogFrameDlg::GetOperResult(UINT8 result, UINT8 unLogType)
{
    QString reString = "成功";

	if(unLogType == OPER_CTRL_REST_APOUR ||\
	   unLogType == OPER_CTRL_REST_VPOUR ||\
	   unLogType == OPER_CTRL_REST_WATER ||\
	   unLogType == OPER_SYS_TYPE_REST)
	{
        reString = "灌注";
		if(result == 0)
		{
...
QString MonLogFrameDlg::GetOperType(UINT8 type, UINT8 unLogType)
{
    ...
    if(unLogType == OPER_CTRL_TYPE_APOUR||\
	   unLogType == OPER_CTRL_TYPE_VPOUR)
    {
        reString = "灌注";
		if(type == 0)
		{
	...
	else if(unLogType == OPER_CTRL_REST_APOUR ||\
		    unLogType == OPER_CTRL_REST_VPOUR ||\
		    unLogType == OPER_CTRL_REST_WATER ||\
		    unLogType == OPER_SYS_TYPE_REST)
	{
	...
	else if(unLogType == OPER_CTRL_SYS_UP ||\
		    unLogType == OPER_CTRL_UP_APOUR ||\
		    unLogType == OPER_CTRL_UP_VPOUR ||\
		    unLogType == OPER_CTRL_UP_WATER)
	{
        reString = "升级";
		if(type == 0)
		{
...
void MonLogFrameDlg::HideOrShowWidget(bool tFlag)
{
	...
	switch(m_unWidgetFlag)
	{
		...
		case OPER_LOG_TYPE_WAT:
			...
			break;
		case OPER_LOG_TYPE_APUM:
		case OPER_LOG_TYPE_VPUM:
			ShowPumLogWidget(tFlag);
			break;	
		case OPER_CTRL_TYPE_APUM:
		case OPER_CTRL_TYPE_VPUM:	
		case OPER_CTRL_TYPE_WAT:
		case OPER_CTRL_TYPE_APOUR:
		case OPER_CTRL_TYPE_VPOUR:
		case OPER_CTRL_REST_APOUR:
		case OPER_CTRL_REST_VPOUR:
		case OPER_CTRL_REST_WATER:
		case OPER_SYS_TYPE_REST:
		case OPER_CTRL_SYS_UP:
	...
	switch(logType)
	{
	    ...
	    case OPER_LOG_TYPE_ALM:
            ...
            for(int i = 0; i < 2; i++)
            {
                ...
                m_pAlmUTmpA[i]->setText(ValueToString(operLogAlm->unAlmUTmpA[i],10,1));
				SetTextShowColor(m_pAlmUTmpA[i], (UINT32)operLogAlm->unAlmUTmpA[0],\
					                              (UINT32)operLogAlm->unAlmUTmpA[1]);
				...
				m_pAlmBubblA[i]->setText(QString("%1").arg(operLogAlm->unAlmBubbleA[i]));
				SetTextShowColor(m_pAlmBubblA[i], (UINT32)operLogAlm->unAlmBubbleA[0],\
					                              (UINT32)operLogAlm->unAlmBubbleA[1]);
				...
				m_pAlmUTmpV[i]->setText(ValueToString(operLogAlm->unAlmUTmpV[i],10,1));
				SetTextShowColor(m_pAlmUTmpV[i], (UINT32)operLogAlm->unAlmUTmpV[0],\
					                              (UINT32)operLogAlm->unAlmUTmpV[1]);
				...
				m_pAlmBubblV[i]->setText(QString("%1").arg(operLogAlm->unAlmBubbleV[i]));
				SetTextShowColor(m_pAlmBubblV[i], (UINT32)operLogAlm->unAlmBubbleV[0],\
					                              (UINT32)operLogAlm->unAlmBubbleV[1]);
		...
		case OPER_LOG_TYPE_POUR:
            ...
            for(int i = 0; i < 2; i++)
            {
				...
				m_pPourUTmpA[i]->setText(ValueToString(operLogPour->unPourUTmpA[i],10,1));
				SetTextShowColor(m_pPourUTmpA[i], (UINT32)operLogPour->unPourUTmpA[0],\
					                              (UINT32)operLogPour->unPourUTmpA[1]);				
				...
				m_pPourUSpeeV[i]->setText(QString("%1").arg(operLogPour->unPourUSpeeV[i]));
				SetTextShowColor(m_pPourUSpeeV[i], (UINT32)operLogPour->unPourUSpeeV[0],\
					                              (UINT32)operLogPour->unPourUSpeeV[1]);
				
				m_pPourUTmpV[i]->setText(ValueToString(operLogPour->unPourUTmpV[i],10,1));
				SetTextShowColor(m_pPourUTmpV[i], (UINT32)operLogPour->unPourUTmpV[0],\
					                              (UINT32)operLogPour->unPourUTmpV[1]);				
		...
		case OPER_LOG_TYPE_WAT:
			...
			for(int i = 0; i < 2; i ++)
			{
				...
				m_pWatTmpValLimt[i]->setText(ValueToString(operLogWatDev->unTmpValLimt[i],10,1));
				SetTextShowColor(m_pWatTmpValLimt[i], (UINT32)operLogWatDev->unTmpValLimt[0],\
					                              (UINT32)operLogWatDev->unTmpValLimt[1]);
		...
		case OPER_LOG_TYPE_VPUM:	
			...
			if(logType == OPER_LOG_TYPE_APUM)
				m_pLatPumType->setText("动脉血泵模块");
			else
				m_pLatPumType->setText("静脉血泵模块");
			for(int i = 0; i < 2; i ++)
			{
				...
				m_pPumPreeValLimt[i]->setText(ValueToString(operLogPumDev->unPressLimt[i],10,1));
				SetTextShowColor(m_pPumPreeValLimt[i], (UINT32)operLogPumDev->unPressLimt[0],\
												(UINT32)operLogPumDev->unPressLimt[1]);

				m_pPumSpeeValLimt[i]->setText(QString("%1").arg(operLogPumDev->unSpeedLimt[i]));
				SetTextShowColor(m_pPumSpeeValLimt[i], (UINT32)operLogPumDev->unSpeedLimt[0],\
												(UINT32)operLogPumDev->unSpeedLimt[1]);
			}
			break;
			/**/
		case OPER_CTRL_TYPE_APUM:
		...
		case OPER_CTRL_SYS_UP:
			...
			if(OPER_CTRL_TYPE_APUM == logType)
			{
			    tmpStr = "动脉血泵开关";
			}
			if(OPER_CTRL_TYPE_VPUM == logType)
			{
			    tmpStr = "静脉血泵开关";
			}			
			...
			else if(OPER_CTRL_TYPE_APOUR == logType)
			{
			    tmpStr = "动脉灌注模式";
			}
			else if(OPER_CTRL_TYPE_VPOUR == logType)
			{
			    tmpStr = "静脉灌注模式";
			}
			else if(OPER_CTRL_REST_APOUR == logType)
			{
			    tmpStr = "动脉重启";
			}
			else if(OPER_CTRL_REST_VPOUR == logType)
			{
			    tmpStr = "静脉重启";
			}
			else if(OPER_CTRL_REST_WATER == logType)
			{
			    tmpStr = "水浴重启";
			}
			...
			else if(OPER_CTRL_SYS_UP == logType)
			{
			    tmpStr = "上位机升级";
			}				
            ...
            for(int i = 0; i < 2; i++)
            {
				...
				m_pResult[i]->setText(GetOperResult(operLogCtl->unResult[i], logType));
...
void MonLogFrameDlg::SetTextShowColor(QLineEdit* pLineEdit, UINT32 unNmb1, UINT32 unNmb2)
{
	if(unNmb1 != unNmb2)
	{
		pLineEdit->setStyleSheet("background-color:rgb(255,204,143); color: rgb(0, 0, 0);");
	}
	else
	{
	   pLineEdit->setStyleSheet("background-color:rgb(54,54,61); color: rgb(255, 255, 255);");//("background-color:lightblue");
	}
}
...
*/
		...
		m_pTblViewTestDetail->setStyleSheet("background-color:rgb(38, 38, 44);");
		m_pTblViewTestDetail->horizontalHeader()->setStyleSheet("QHeaderView::section{background-color:#36363D;color:white;font:24px;}");  //行表头
		m_pTblViewTestDetail->verticalHeader()->setVisible(false);
    ...
    for(lRowIndex = 0; lRowIndex < g_MonSaveInfo.unMonDataNum; lRowIndex++)
    {
        ...
        pNewItem = new QStandardItem(DataTypeToString(g_DataTests[lRowIndex].unADataType));
        pNewItem->setSelectable(false);
        pNewItem->setEnabled(false);
        pItemModel->setItem(lRowIndex ,lColIndex++, pNewItem);
        ...
        pNewItem = new QStandardItem(DataTypeToString(g_DataTests[lRowIndex].unVDataType));
        pNewItem->setSelectable(false);
        pNewItem->setEnabled(false);
        pItemModel->setItem(lRowIndex ,lColIndex++, pNewItem);
		
        pNewItem = new QStandardItem(ValueToString(g_DataTests[lRowIndex].unVPrssSet,10,1));
        pNewItem->setSelectable(false);
        pNewItem->setEnabled(false);
        pItemModel->setItem(lRowIndex ,lColIndex++, pNewItem);
		
	    pNewItem = new QStandardItem(ValueToString(g_DataTests[lRowIndex].unVPrssVal,10,1));
        pNewItem->setSelectable(false);
        pNewItem->setEnabled(false);
        pItemModel->setItem(lRowIndex ,lColIndex++, pNewItem);
		
        pNewItem = new QStandardItem(ValueToString(g_DataTests[lRowIndex].unVSpeeSet,1,0));
        pNewItem->setSelectable(false);
        pNewItem->setEnabled(false);
        pItemModel->setItem(lRowIndex ,lColIndex++, pNewItem);
		
	    pNewItem = new QStandardItem(ValueToString(g_DataTests[lRowIndex].unVSpeeVal,1,0));
        pNewItem->setSelectable(false);
        pNewItem->setEnabled(false);
        pItemModel->setItem(lRowIndex ,lColIndex++, pNewItem);
		
        pNewItem = new QStandardItem(ValueToString(g_DataTests[lRowIndex].unVFlowVal,1,0));
        pNewItem->setSelectable(false);
        pNewItem->setEnabled(false);
        pItemModel->setItem(lRowIndex ,lColIndex++, pNewItem);
		
        pNewItem = new QStandardItem(ValueToString(g_DataTests[lRowIndex].unVTmpVal,10,1));
        pNewItem->setSelectable(false);
        pNewItem->setEnabled(false);
        pItemModel->setItem(lRowIndex ,lColIndex++, pNewItem);
    ...
    m_pTblViewTestDetail->sortByColumn(0,Qt::DescendingOrder);
