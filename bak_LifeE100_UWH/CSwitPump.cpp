        ...
        if(false==GetNextCmdData()&&0==m_unCmdMsTimer)
        {
            if(true==m_bAtvFlag)
            {
                if(false == m_bUpDveFlag)
                {
					...
					else if(true == m_bSndAlmFlg)
					{
					    SndAlmState();
						m_bSndAlmFlg = false;
					}
					else if(true == m_bSndBatFlg)
					{
						SndBatState();
						m_bSndBatFlg = false;
					}
...
void CSwitPump::SetAlmLedState(UINT8 unAlmSta)
{
    printf("entr CSwitPump::SetAlmLedState unDevID:%lu unStat:%u\r\n",m_unDevID,unAlmSta);
	m_unAlmState = unAlmSta;
	m_bSndAlmFlg = true;
}
//发送告警级别状态
bool CSwitPump::SndAlmState(void) 
{
    printf("entr CSwitPump::SndAlmState unDevID:%lu \r\n",m_unDevID);
    if(false==BuillCmdData((BYTE*)&m_unAlmState,sizeof(m_unAlmState),CMD_SND_ALM_STA))
    {
        ClearCmds();
        BuillCmdData((BYTE*)&m_unAlmState,sizeof(m_unAlmState),CMD_SND_ALM_STA);
    }
    SetCmdCtrlStat(CTRL_ALM_STA_SND,CMD_ST_SND);
    return true;
}
...
void CSwitPump::DataParser(BYTE* pDataBuf,UINT32 unCmdID,UINT32 unDataLen)
{
    ...
    switch(unCmdID)
    {
		...
		if(unDataLen <= sizeof(PumpDevData))
		{
			...
			m_pPumpData.unBubbVal = pDevData->unBubbVal;
			m_pPumpData.unDevCode = htonl(pDevData->unDevCode);
            m_lckMutex.unlock();
            m_bGetData = true;
			//if(m_unDevID == VPUM_DEV_BAS_ID)
			//printf("CSwitPump::%d-----unRunFlg:%u unPourSta:%u unDevCode:%x\r\n",m_unDevID,m_pPumpData.unRunFlg,m_pPumpData.unPourSta,m_pPumpData.unDevCode);
			//printf("unPrssVal:%d unSpeeVal:%u unTmpVal:%x\r\n",m_pPumpData.unPrssVal,m_pPumpData.unSpeeVal,m_pPumpData.unTmpVal);
			...
			//printf("unDataLen:%lu \r\n",unDataLen);
			//for(UINT32 i = 0; i < unDataLen; i++)
				//printf("pDataBuf[%lu] = %x \r\n",i,pDataBuf[i]);
		}
		else
		{
		...
		case CMD_SND_PUL_PRM_ACK:
			...
			break;
		case CMD_CAL_ZERO_DEV_ACK:
			SetCmdCtrlStat(CTRL_CAL_ZERO_SND,CMD_ST_CFM);
			m_unCalZeroBak = CAL_SET_OK;
			break;
		case CMD_SND_DEV_STA_ACK:
			SetCmdCtrlStat(CTRL_DEV_STA_SND,CMD_ST_CFM);
			printf("CSwitPump::CMD_SND_DEV_STA_ACK  unDevID:%d \r\n",m_unDevID);
			break;
		case CMD_SND_ALM_STA_ACK:
			SetCmdCtrlStat(CTRL_ALM_STA_SND,CMD_ST_CFM);
			printf("CSwitPump::CMD_SND_ALM_STA_ACK  unDevID:%d \r\n",m_unDevID);
			break;
	    case CMD_SND_BAT_STA_ACK:
			printf("CSwitPump::CMD_SND_BAT_STA_ACK  unDevID:%d \r\n",m_unDevID);
	        SetCmdCtrlStat(CTRL_BAT_STA_SND,CMD_ST_CFM);
	        break;				
		case CMD_DEV_IP_ERR_INFO:
			m_lckMutexUpInfo.lock();
			RevDevErrorInfo((char*)pDataBuf+1);
		    m_lckMutexUpInfo.unlock();
			printf("CSwitPump::CMD_DEV_IP_ERR_INFO  unDevID:%d \r\n",m_unDevID);
			//printf("m_unDevID:%d - %s \r\n",m_unDevID,pDataBuf);
			break;			
		default:break;
    }
}
