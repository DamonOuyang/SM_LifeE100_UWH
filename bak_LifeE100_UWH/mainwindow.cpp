...
MainWindow::~MainWindow()
{
	...
	m_pLoadHeadDialog = NULL;
	delete m_pLoadHeadDialog;
...
void MainWindow::FlashWaterRoomInfo(void)
{
    ...
    if(watDevData.unTmpVal >= 100 && watDevData.unTmpVal <= 450) // dm2.6.5
    {
        ui->showTmpLabel->setText(ValueToString(watDevData.unTmpVal,10,1));//温度
    }
    else
    {
        ui->showTmpLabel->setText("--");//温度
    }
}
...
void MainWindow::FlashPumpAInfo(void)
{
    ...
    //转速
    if(pumDevData.unSpeeVal >= 1000 && pumDevData.unSpeeVal <= 10000) // dm2.3.3
    {
	...
	{
	    ui->preSpeedLb->setText("--");
		ui->pourSpeedLb->setText("--");
	}

	//流量
    if(m_unPumpRunA == 1 && (pumDevData.unFlowVal >= -3000 && pumDevData.unFlowVal <= 3000))// dm2.4
	{
	...
	//温度
    if(pumDevData.unTmpVal >= 100 && pumDevData.unTmpVal <= 450)// dm2.5
	{
	...
	{
	    ui->preTmpLb->setText("--");
		ui->pourTmpLb->setText("--");
	}
	//压力
    if(pumDevData.unPrssVal >= -300 && pumDevData.unPrssVal <= 2000) // dm2.1.2
	{
	...
	{
		ui->prePressLb->setText("--");
		ui->pourPressLb->setText("--");	
	}
...
void MainWindow::FlashPumpVInfo(void)
{
    ...
    //转速
    if(pumDevData.unSpeeVal >= 1000 && pumDevData.unSpeeVal <= 10000)
    {
	...
	{
	    ui->preSpeedLbV->setText("--");
		ui->pourSpeedLbV->setText("--");
	}
	//流量
	if(m_unPumpRunA == 1 && (pumDevData.unFlowVal >= -3000 && pumDevData.unFlowVal <= 3000))
	{
	...
	//温度
	if(pumDevData.unTmpVal >= 0 && pumDevData.unTmpVal <= 450)
	{
	...
	{
	    ui->preTmpLbV->setText("--");
		ui->pourTmpLbV->setText("--");
	}
	//压力
	if(pumDevData.unPrssVal >= -300 && pumDevData.unPrssVal <= 2000)
	{
		if(pumDevData.unPrssVal < 0)
		{
		    ui->prePressLbV->setText(ValueToString((pumDevData.unPrssVal-5)/10,1,0));
			ui->pourPressLbV->setText(ValueToString((pumDevData.unPrssVal-5)/10,1,0));
		}
		else
		{
			ui->prePressLbV->setText(ValueToString((pumDevData.unPrssVal+5)/10,1,0));
	...
	{
		ui->prePressLbV->setText("--");
		ui->pourPressLbV->setText("--");	
	}	
...
void MainWindow::PourPressLEClick(void)
{
	if(m_pCurLineEdit == NULL)
	{
        ...
        printf("g_devConfigInfo.arteryParm.unPressLimt = %d\n",g_devConfigInfo.arteryParm.unPressLimt);
	    SetValDialog  setValDialog(this);
		setValDialog.setGeometry(QRect(0, 40, SET_VAL_DLG_W, SET_VAL_DLG_H));
	    setValDialog.SetDefaultVal(ui->pourPressSetLE->text(),ART_PRE_SET_TITLE);
        setValDialog.SetValLimit(SET_PRESS_LIMIT_MIN,g_devConfigInfo.arteryParm.unPressLimt,SET_STEP_1,1,0,1); // dm
	    setValDialog.setWindowModality(Qt::ApplicationModal);
	    setValDialog.exec();
	}
	else
...
//        setValDialog.SetValLimit(PRE_POUR_SPEED_LIMIT_MIN,g_devConfigInfo.arteryParm.unSpeedLimt,SET_STEP_1,1,0,10);
        setValDialog.SetValLimit(PRE_POUR_SPEED_LIMIT_MIN,PRE_POUR_SPEED_LIMIT_MAX,SET_STEP_1,1,0,10); // dm
	    setValDialog.setWindowModality(Qt::ApplicationModal);
	    setValDialog.exec();
    }
	else
...
void MainWindow::FlashBattryInfo(void)
{
    ...
    ShowACLinkLog(bDisChg); //dm2.8.1
    g_devAlmBits.BatAlm.bACLosBits = bDisChg; // dm
...
/*
    ...
    */
    if(bLinkFlg == true)
	{
		ui->batLinkLog->setStyleSheet(QLatin1String("#batLinkLog{\n"
	...
	{
		ui->batLinkLog->setStyleSheet(QLatin1String("#batLinkLog{\n"
		"background-image: url(:/images/batteryUnlink_Icon.png);\n"
		"}"));
	}
...
/*
    ...
    */
    if(bLinkFlg == true)
	{
		ui->acLinkLog->setStyleSheet(QLatin1String("#acLinkLog{\n"
		"background-image: url(:/images/acUnlink_Icon.png);\n"
		"}"));
	}
	else
	{
		ui->acLinkLog->setStyleSheet(QLatin1String("#acLinkLog{\n"
		"background-image: url(:/images/acLink_Icon.png);\n"
		"}"));
	}
...
void MainWindow::on_monTabWidget_currentChanged(int index)
{
    printf("----%d\n",index);
    if(index == 1) // 1 pourPressSetLE
    {
        ui->pourPressSetLE->setText("30");
        ui->pourPressSetLE->update();
    }
    else // 0
    {
