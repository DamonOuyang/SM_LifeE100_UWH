...
CSwitModule::CSwitModule()
{
	...
	memset(&m_pDevErrInfo,0,sizeof(m_pDevErrInfo));
    m_bSetPrmFlag = false;
	m_unGetVerEn = false;
	m_unTotalSize = 0;
	m_unTotalPkg = 0;
...
void CSwitModule::SetBatState(UINT8 unBatSta)
{
    printf("entr CSwitModule::SetBatState unDevID:%lu unStat:%u\r\n",m_unDevID,unBatSta);
	m_unBatState = unBatSta;
	m_bSndBatFlg = true;
}
//发送告警级别状态
bool CSwitModule::SndBatState(void) 
{
    printf("entr CSwitModule::SndBatState unDevID:%lu \r\n",m_unDevID);
    if(false==BuillCmdData((BYTE*)&m_unBatState,sizeof(m_unBatState),CMD_SND_BAT_STA))
    {
        ClearCmds();
        BuillCmdData((BYTE*)&m_unBatState,sizeof(m_unBatState),CMD_SND_BAT_STA);
    }
    SetCmdCtrlStat(CTRL_BAT_STA_SND,CMD_ST_SND);
    return true;
}
...
bool CSwitModule::GetErrInfiFlag(void)
{
    return m_bErrInfoFlg;
}
void CSwitModule::GetErrInfo(char *pGetBuf)
{
	memcpy(pGetBuf,(char*)&m_pDevErrInfo,sizeof(m_pDevErrInfo));
    memset(&m_pDevErrInfo,0,sizeof(m_pDevErrInfo));
	m_bErrInfoFlg = false;
}