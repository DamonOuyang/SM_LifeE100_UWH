...
void InitDefParam(void)
{
    ...
    g_devConfigInfo.devAlmParam.unAlmUPresV = 180;
    ...
    g_devConfigInfo.devPourParam.unPourUFlowV = 2000;
...
void LoadCompanyInfo(QString& strCoInfo)
{
    ...
    {
        QString strProdName = "肝脏灌注机Life-X100";
		QString strDesigner = "注册人/生产企业：奇点医疗科技（广州）有限公司";
		QString strOfficeAddr = "住所：广州市增城区宁西街香山大道51号3栋4层405室";
        QString strAddrName = "生产地址：广州市增城区宁西街香山大道51号5栋5层";
		QString strPostcode = "邮 编：511338";
        QString strCnnPhone = "电话:020-37602835";
...
QString CheckOneAlmShow(UINT8 *unGetLevel)
{
    ...
    if(g_devAlmBits.BatAlm.bBatSocBits) // dm2.8.2
	{
        printf("dmmmmmmmdmmmmmmmmmmdmmmmmmmmmmmmmmmmmmmmmmdmmmmmmmmmmmmmmm2\n");
	    *unGetLevel = ALM_LEV_H;
		strAlmItem = "电池电量低";
		return strAlmItem;
	}		
    ...
    if(g_devAlmBits.BatAlm.bACLosBits) // dm2.8.1
	{
	    *unGetLevel = ALM_LEV_M;
		strAlmItem = "电源线未连接";
		return strAlmItem;
...
void CleanDevAlm(UINT8 unDevtype)
{
    ...
    {
        memset(&g_devAlmBits.VpumGrpAlm,0,sizeof(PumGrpAlmBits));
    }
    else if(unDevtype == DEV_VPUMP_TYPE)
    {
        memset(&g_devAlmBits.ApumGrpAlm,0,sizeof(PumGrpAlmBits));
    }
    else if(unDevtype == DEV_WAT_TYPE)
    {
        memset(&g_devAlmBits.watGrpAlm,0,sizeof(WatGrpAlmBits));
    }
}
