...
void configVeiFrame::OnClickOK(void)
{
    ...
    g_devConfigInfo_Edt.veinParm.unPressLimt  = GetItemValue(m_pPressLimt,10)>SET_PRESS_LIMIT_V?SET_PRESS_LIMIT_V:GetItemValue(m_pPressLimt,10); // dm
...
void configVeiFrame::OnClickCancel(void)
{
	OnUpdParams();
}

void configVeiFrame::confirmString(QString strIn)
{
...
void configVeiFrame::OnItemEdit(void)
{
    if(NULL!=m_pCurLineEdit)
    {
...
void configVeiFrame::OnFocusChanged(QWidget * old, QWidget * now)
{
    if(m_pPress == old ||\
	   m_pSpeed == old ||
	   m_pPressLimt == old ||\
...
void configVeiFrame::OnUpdParams(void)
{
