...
void CWorkThread::VeinPrePourAlm(void)
{
	...
	//静脉流量传感器未连接-技术
	unAlmBits = m_procPour.GetDevAlmState(VPUM_DEV_BAS_ID, PUM_FLO_LOS_BITS);
	if(unAlmBits == 1)
	{
	...
	//静脉血温传感器未连接
	unAlmBits = m_procPour.GetDevAlmState(VPUM_DEV_BAS_ID, PUM_TMP_LOS_BITS);
	if(unAlmBits == 1)
	{
			 ...
			 g_pourTstData.tstData.unWatTmpVal)>MAX_ALM_WAT_TMP_DIF)
	{
		...
		g_pourTstData.tstData.unWatPreVal < PRESS_ERR_VAL_W)
	 {
		 if(g_devAlmBits.watGrpAlm.bWatPreUBits == false)
		 {
			 SetAlmItem(g_devAlmBits.watGrpAlm.bWatPreUBits,\
						H_T_ALM_ITEM_WAT_PPRE_UP,WAT_DEV_BAS_ID,0x00,\
						g_pourTstData.tstData.unWatPreVal,1);
			 g_DataAlarm.unWatPreVal = g_pourTstData.tstData.unWatPreVal;
			  ...
			  g_pourTstData.tstData.unWatPreVal)>MAX_ALM_WAT_PRE_DIF)
	 {
		 if(g_devAlmBits.watGrpAlm.bWatPreUBits == true)
		 {
			 SetAlmItem(g_devAlmBits.watGrpAlm.bWatPreUBits,\
						H_T_ALM_ITEM_WAT_PPRE_UP,WAT_DEV_BAS_ID,0x00,\
						g_pourTstData.tstData.unWatPreVal,0);
			 g_DataAlarm.unWatPreVal = g_pourTstData.tstData.unWatPreVal;
...
void CWorkThread::OnMsTimer(UINT32 unMsCount)
{
    ...
    else
    {
		...
		m_pMcuUpInfo.unTotalPkg = 56;
		m_pMcuUpInfo.unPkgIndex++;
		if(m_pMcuUpInfo.unPkgIndex >= 56)
		{
			m_pMcuUpInfo.unMcuUpdState = UpdSuccess;
			m_pMcuUpInfo.unPkgIndex = 0;
		}
		else
			m_pMcuUpInfo.unMcuUpdState = UpdDoing;
