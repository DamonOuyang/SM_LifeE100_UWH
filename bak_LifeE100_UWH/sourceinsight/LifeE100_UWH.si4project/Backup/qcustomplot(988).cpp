  ...
  // generate subticks between ticks:
	...
	if(mAutoTickLabels)
	{
		...
		else if(mTickLabelType == ltDateTime)
		{
			for(int i = mLowestVisibleTick; i <= mHighestVisibleTick; ++i)
			{
			    // use fromMSecsSinceEpoch function if available, to gain sub-second accuracy on tick labels (e.g. for format "hh:mm:ss:zzz")
				#if QT_VERSION < QT_VERSION_CHECK(4, 7, 0)
  ...
  mBaseValue(0)
{
  ...
  mSelectedPen.setColor(QColor(80, 80, 255)); // lighter than Qt::blue of mPen
  mSelectedBrush = mBrush;
  m_penTooltip.setColor(Qt::red);
  m_penTooltip.setStyle(Qt::SolidLine);
  m_bShowBarValFlag = false;
  m_dBarValTextRotation = 0;
}
...
void QCPBars::setTooltipPen(QPen pen)
{
    m_penTooltip = pen;
}
...
void QCPBars::draw(QCPPainter *painter)
{
  ...
  for (it = lower; it != upperEnd; ++it)
  {
    ...
    if (mainBrush().style() != Qt::NoBrush && mainBrush().color().alpha() != 0)
    {
      ...
      //painter->drawText(barPolygon.at(0),"TestValue");
    }
    // draw bar line:
    ...
    if(true==m_bShowBarValFlag)
    {
        QPen penOld = painter->pen();
        painter->setPen(m_penTooltip);
        DrawBarValText(painter,barPolygon.at(1),QString("%1").arg(it.value().value));
        painter->setPen(penOld);
    }
  }
}
void QCPBars::SetShowBarValText(bool bShowBarValFlag,double dbDegree)
{
    m_bShowBarValFlag = bShowBarValFlag;
    m_dBarValTextRotation = dbDegree;
}

void QCPBars::DrawBarValText(QCPPainter *painter,QPointF pPointF,QString strValText)
{
    // backup painter settings that we're about to change:
    QTransform oldTransform = painter->transform();
    QFont oldFont = painter->font();
    ...
    // transform painter to position/rotation:
    painter->translate(pPointF);
    if (!qFuzzyIsNull(m_dBarValTextRotation))
      painter->rotate(m_dBarValTextRotation);

    QFont font("sans-serif",9);
    painter->setFont(font);
    painter->drawText(0,0,strValText);
