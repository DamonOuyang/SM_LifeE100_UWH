...
{
    int nRcv = read(GetSocket(),m_pRcvBuf+TmpLen, MAX_RCV_BUF_SIZE);

	/**/
	printf("-nRcv:%d----m_pRcvBuf[0]:%02X-------------sock:%d------------------------------\r\n",nRcv,m_pRcvBuf[0],GetSocket());
		
    for(UINT8 i = 0; i < TmpLen; i++)
    {
    ...
    if (nRcv > 0)
    {
		...
		if(m_pRcvBuf[TmpLen] != 0x77)
		{
			...
			else if(m_pRcvBuf[6] == 0x77)
			{
			    TmpLen -= 7;
				for(UINT8 i = 0; i < TmpLen; i++)
				{
				    m_pRcvBuf[i] = m_pRcvBuf[7+i];
				}
				printf("rcv-----------------------7-------------------\r\n");
			}
			return;
		}
		else if(m_pRcvBuf[TmpLen] == 0x77)
		{
		    if(TmpLen <= 8)
		    {
		        TmpLen = 0;
	...
	printf("\r\n");			
        ...
        UINT16 unFrmPos = 0;
        UINT16 unEndPos = TmpLen;
        while(unFrmPos < unEndPos)
        {
            RecvDataFrmUART(m_pRcvBuf[unFrmPos]);
            unFrmPos++;
        }
		printf("unFrmPos = %d ---unDataPos = %d\r\n",unFrmPos,m_uartDataBuf.unDataPos);
...
/*
    ...
    UINT16 tmpChkSum = CreateJBDCheckSum(pDataBuf+2,unDataSize-4);
	
    if(unCheckSum != tmpChkSum)
    {
        printf("CheckSum Error :unCheckSum=%u tmpChkSum:%u\r\n",unCheckSum,tmpChkSum);
