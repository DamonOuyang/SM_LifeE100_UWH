    ...
    QFrame(parent)
{
	...
	AddItem(formLayout,m_pPourUTmpA,tr("血温上限:"),tr("(℃)"),lEditWidth);
	...
	pFirstRow->addLayout(pFrmLayout);
	formLayout->addRow(pFirstRow);	
	AddItem(formLayout,m_pPourUTmpV,tr("血温上限:"),tr("(℃)"),lEditWidth);
...
void PourConfigPanel::OnClickOK(void)
{
    ...
    //压力保护值比告警值大10
	if(g_devConfigInfo_Edt.devPourParam.unPourUPresA < (g_devConfigInfo.devAlmParam.unAlmUPresA+50))
	{
		g_devConfigInfo_Edt.devPourParam.unPourUPresA = g_devConfigInfo.devAlmParam.unAlmUPresA+50;
	...
	g_devConfigInfo_Edt.devPourParam.unPourUTmpA = GetItemValue(m_pPourUTmpA,10);
	...
	g_devConfigInfo_Edt.devPourParam.unPourUTmpV = GetItemValue(m_pPourUTmpV,10);
