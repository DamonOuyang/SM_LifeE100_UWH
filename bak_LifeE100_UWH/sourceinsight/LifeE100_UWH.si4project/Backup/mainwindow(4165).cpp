...
void MainWindow::LoadConfigFile(void)
{
    ...
    g_devConfigInfo.veinParm.unSpeed = 2000;
...
{
    printf("Entr OnMonTabChangedA Tab:%d \r\n",ui->monTabWidget->currentIndex());
...
void MainWindow::CheckAutoPress(void)
{
    ...
    if(g_devAlmBits.watGrpAlm[0].bBloodBit == 1)
    {
		...
		{
		    SysAutoChangePress(DEV_VPUMP_TYPE, SET_AUTO_PRESS-300);
		}
    }
}
    ...
    if(m_bSysReStrFlg == true)
    {
		...
		if(m_unPumpRunV == 1)
		{
		    if(0 == ui->BPStrBtnV->isChecked())
		    {
				...
				ui->monTabWidget->setCurrentIndex(m_unPourStaV);//启动血泵灌注状态与下位机一致
		    }
		}
		//水浴
...
void MainWindow::CheckPourPrmA(void)
{
	...
	{
		operLogPumDev.unPress[1] = SET_POUR_PRESS;
		g_devConfigInfo_Edt.arteryParm.unPress = SET_POUR_PRESS;
	}
	else
	{
	...
	{
		operLogPumDev.unSpeed[1] = SET_POUR_SPEED;
		g_devConfigInfo_Edt.arteryParm.unSpeed = SET_POUR_SPEED;
	}
	else
	{
...
void MainWindow::CheckWatStop(void)
{
    if(g_devAlmBits.watGrpAlm[0].bTmpUBits||\
	   g_devAlmBits.ApumGrpAlm[0].bTmpErrBits||g_devAlmBits.VpumGrpAlm[1].bTmpErrBits||\
...
void MainWindow::CheckPourPrmV(void)
{
	...
	{
		operLogPumDev.unPress[1] = SET_POUR_PRESS;
		g_devConfigInfo_Edt.veinParm.unPress = SET_POUR_PRESS;
	}
	else
	{
	...
	{
		operLogPumDev.unSpeed[1] = SET_POUR_SPEED;
		g_devConfigInfo_Edt.veinParm.unSpeed = SET_POUR_SPEED;
	}
	else
	{
...
{
    printf("Entr OnMonTabChanged-------V Tab:%d \r\n",ui->VmonTabWidget->currentIndex());
