...
void CWorkThread::ProcUsrSrtCmd(void)
{
	...
	operLogCtl.unType = OPER_CTRL_TYPE_PUM;
	operLogCtl.unSite = 0;
    ...
    if(USR_BP_CMD_STRT == unUserCmdID)
    {
		...
		SetDevPrm setDevPrm;
		setDevPrm.unIndex = CFG_ARTERY_SPD_ITEM;
    ...
    /**/
        ...
        {
            //启动血泵
            if(false==m_procPour.StartRun(m_unBpGrpID))
            {
    ...
    else if(USR_BP_CMD_STOP==unUserCmdID)
    {
        ...
        m_procPour.StopPump(STP_ERR_USR_CNL_ERR);
    }
...
void CWorkThread::InitThread(void)
{
	...
	//串口对应板子---p5
    //if(false==pUartSkt->OpenDev((char*)"/dev/ttyUSB0",115200))
    ...
    //if(false==pUartSkt->OpenDev((char*)"/dev/ttyUSB1",115200))
    if(false==pUartSkt->OpenDev((char*)"/dev/ttyAMA2",115200))
    {
        printf("Open CommPort:%s FAIL!\r\n","/dev/ttyAMA2");
        delete pUartSkt;
    }
	...
	//对应板子---p2
	pUartSkt = new CUARTSocket(m_pSktHandler);
    if(false==pUartSkt->OpenDev((char*)"/dev/ttyAMA3",115200))
    {
        printf("Open CommPort:%s FAIL!\r\n","/dev/ttyAMA3");
        delete pUartSkt;
    }
...
void CWorkThread::CreateVeinMods(void)
{
    ...
    for(lIndex = 0; lIndex<g_devConfigInfo.basicCfgInfo.unVPumCount; lIndex++)
    {
		...
		pSwitPum->SetSndID(VPUM_DEV_BAS_ID);
...
void CWorkThread::SetDataType(UINT8     unPumType, UINT8 unDataType)
{
	...
	operLogCtl.unType = OPER_CTRL_TYPE_POUR;
	operLogCtl.unSite = 0;
	GetDateTime(&operLogCtl.dtCreate);
	operLogCtl.unResult[0] = 1;
	operLogCtl.unResult[1] = 1;
	
	operLogCtl.unOperType[0] = m_unDataType;
	operLogCtl.unOperType[1] = unDataType;
	SaveOperLogData((const char*)&operLogCtl, sizeof(OperLogCtl));
	SaveOperLogInfo();	

    m_unDataType = unDataType;
...
/**/
void CWorkThread::StartBloodPump(UINT8 unBpGrpID)
{
    //用户手动启动或网络启动或定时启动血泵
...
void CWorkThread::StopBloodPump(void)
{
...
void CWorkThread::SetPourState(UINT8      unPumType, UINT8 unIndex)
{
	...
	pSwitPum->SetPourState(m_unDataType);
}
...
bool CWorkThread::IsBPRun(void)
{
    bool bRunFlag;
    m_lckMutex.lock();
    bRunFlag = m_bBpRunFlag;
    m_lckMutex.unlock();
    return bRunFlag;
}
*/
...
bool CWorkThread::GetUpdConfigInfo(void)
{
		    ...
		    unItemIndex==CFG_ARTERY_FLW_ITEM)
    {
		...
		/*
		if(unItemIndex==CFG_ARTERY_PRS_ITEM)
		{
		    m_unArteryPrmIndex = 2;
		}
		else if(unItemIndex==CFG_ARTERY_SPD_ITEM)
		{
		    m_unArteryPrmIndex = 1;
		}
		*/
...
/**/
void CWorkThread::OnPumStop(void)
{
    m_lckMutex.lock();
    m_bBpRunFlag = false;
    m_lckMutex.unlock();
}
bool CWorkThread::IsPumpRuning(void)
{
    return m_bBpRunFlag;
}
...
void CWorkThread::ProcAlmInfo(void)
{
    ...
    for(unIndex = 0; unIndex < g_devConfigInfo.basicCfgInfo.unAPumCount; unIndex++)
    {
        ...
        {
            //流量告警 泵开，等待5秒
            if(m_bBpRunFlag == true && m_unAlmFlowDlySec > 5)
            {
...
UINT32 CWorkThread::GetDevData(UINT8      untype, UINT8 unIndex, BYTE* pDataBuf,UINT32 unBufSize)
{
    ...
    {
		unReturn = m_procPour.GetPumpData(APUM_DEV_BAS_ID,unIndex,pDataBuf,unBufSize);
    }
