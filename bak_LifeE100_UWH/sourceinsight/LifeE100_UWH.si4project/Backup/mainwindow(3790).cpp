...
void MainWindow::FlashPumpAInfo(void)
{
    ...
    m_unPumpRunA = 1;//pumDevData.unRunFlg;
	m_unPourStaA = 1;//pumDevData.unPourSta;
...
void MainWindow::FlashBattryInfo(void)
{
    ...
    //{
		...
		//return ;
    //}
	BatBascInfo batDevData;
	memset(&batDevData,0,sizeof(batDevData));
	g_pWorkThread->GetDevData(DEV_BAT_TYPE, 0, (BYTE*)&batDevData,sizeof(BatBascInfo));
    ...
    if(batDevData.unBatCur == 0)
		...
		m_unBatStat = 2;//放电
	batDevData.unRSOC = 100;

    ui->batWidget->setValue(batDevData.unRSOC);
	ui->batSocLabel->setText(ValueToString(batDevData.unRSOC,1,0)+"%");
	//ui->showTmpLabel->setText(ValueToString(watDevData.unTmpVal,10,1));//温度
}
