...
void CSwitModule::SetCalZeroCmd(void)
{
    printf("---------CSwitModule::SetCalZeroCmd-----DevId:%d \r\n",m_unDevID);
	m_bCalZeroFlag = true;
}
...
void CSwitModule::RevDevErrorInfo(BYTE *pRevBuf)
{
    if(m_pDevUpErrInfo.unInfoLen == 0)
    {
	...
	{
	    if(m_pDevUpErrInfo.unInfoLen+pRevBuf[0] < MAX_INFO_SIZE)
	    {
	        memcpy(&m_pDevUpErrInfo.pDevErrInfo+m_pDevUpErrInfo.unInfoLen, pRevBuf+1,pRevBuf[0]);
			m_pDevUpErrInfo.unInfoLen += pRevBuf[0];
	    }
		else
		{
		    printf("Err-------devID:%d unInfoLen+pRevBuf[0]:%d \r\n",m_unDevID, m_pDevUpErrInfo.unInfoLen+pRevBuf[0]);
		}
	}

	printf("DevId:%d info:%s \r\n",m_unDevID, (char*)&m_pDevUpErrInfo);
}
