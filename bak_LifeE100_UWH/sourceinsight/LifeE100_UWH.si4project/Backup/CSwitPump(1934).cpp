...
CSwitPump::CSwitPump()
{
	...
	memset(&m_pSetPumSta,0,sizeof(m_pSetPumSta));
    m_bGetData = false;
	m_unGetVerFlg = false;
	m_bStrInitFlg = false;
	m_unPourState = 0;
...
void CSwitPump::DataParser(BYTE* pDataBuf,UINT32 unCmdID,UINT32 unDataLen)
{
    ...
    switch(unCmdID)
    {
		...
		if(unDataLen <= sizeof(PumpDevData))
		{
            ...
            m_lckMutex.unlock();
            m_bGetData = true;
			//printf("CSwitPump::%d-----unRunFlg:%u unPourSta:%u \r\n",m_unDevID,m_pPumpData.unRunFlg,m_pPumpData.unPourSta);
		...
		case CMD_CAL_ZERO_DEV_ACK:
			...
			break;
		case CMD_SND_PUM_STA_ACK:
			SetCmdCtrlStat(CTRL_PUM_STA_SND,CMD_ST_CFM);
			printf("CSwitPump::CMD_SND_PUM_STA_ACK  unDevID:%d \r\n",m_unDevID);
			break;		
		default:break;
    }
}
...
bool CSwitPump::SetPumpState(SetPumSta* pPumSta)
{
	printf("entr CSwitPump::SetPumpState unDevID:%lu unPumSat:%u unPourSta:%u unPumVal:%u\r\n",\
	        m_unDevID,pPumSta->unPumSat,pPumSta->unPourSta,pPumSta->unPumVal);

	memcpy(&m_pSetPumSta,pPumSta,sizeof(pPumSta));
	m_bSetPumSta = true;
	return true;
}

void CSwitPump::SendPumpState(void)
{
    printf("CSwitPump::SendPumpState devID:%lu SndID:%lu\r\n",m_unDevID,m_unSndID);
    m_pSetPumSta.unPumVal = htons(m_pSetPumSta.unPumVal);
    BuillCmdData((BYTE*)&m_pSetPumSta,sizeof(m_pSetPumSta),CMD_SND_PUM_STA);
    SetCmdCtrlStat(CTRL_PUM_STA_SND,CMD_ST_SND);
}
