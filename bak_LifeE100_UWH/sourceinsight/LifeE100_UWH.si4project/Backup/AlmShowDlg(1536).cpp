...
void AlmShowDlg::AddAlmItem(AlmItem* pAlmItem)
{
    ...
    {
        if(ALM_ITEM_WAT_LIN_DN==pAlmItem->unAlmID ||\
		   ALM_ITEM_WAT_LIN_UP==pAlmItem->unAlmID ||\
		   ALM_ITEM_SUB_DEV_TMT==pAlmItem->unAlmID ||\
		   ALM_ITEM_BLOOD_LIN_DN ==pAlmItem->unAlmID)
        {
...
void AlmShowDlg::UpDateRTAlmInfos(void)
{
    ...
    for(unGrpIndex = 0; unGrpIndex < g_devConfigInfo.basicCfgInfo.unAPumCount; unGrpIndex++ )
    {
        ...
        if(g_devAlmBits.ApumGrpAlm[unGrpIndex].bFlowUBits)
        {
            ...
            strItemVal = ValueToString(g_DataAlarm.unAFlowVal,10,1);
            strItemAlmVal = ValueToString(g_devConfigInfo_Edt.devAlmParam.unAlmUFlowA,10,1);
            AddRLAlmItem(unRowIndex++,strAlmItem,strItemVal,strItemAlmVal);
        }
        if(g_devAlmBits.ApumGrpAlm[unGrpIndex].bFlowLBits)
        {
            ...
            strItemVal = ValueToString(g_DataAlarm.unAFlowVal,10,1);
            strItemAlmVal = ValueToString(g_devConfigInfo_Edt.devAlmParam.unAlmLFlowA,10,1);
            AddRLAlmItem(unRowIndex++,strAlmItem,strItemVal,strItemAlmVal);
        }
    ...
    for(unIndex = 0; unIndex < g_devConfigInfo.basicCfgInfo.unVPumCount; unIndex++)
    {
        ...
        {
            strAlmItem = QString("静脉血泵组<%1>---压力高").arg(unGrpIndex+1);
        ...
        {
            strAlmItem = QString("静脉血泵组<%1>---流量高").arg(unGrpIndex+1);
            strItemVal = ValueToString(g_DataAlarm.unVFlowVal,10,1);
            strItemAlmVal = ValueToString(g_devConfigInfo_Edt.devAlmParam.unAlmUFlowV,10,1);
            AddRLAlmItem(unRowIndex++,strAlmItem,strItemVal,strItemAlmVal);
        }
        if(g_devAlmBits.VpumGrpAlm[unGrpIndex].bFlowLBits)
        {
            strAlmItem = QString("静脉血泵组<%1>---流量低").arg(unGrpIndex+1);
            strItemVal = ValueToString(g_DataAlarm.unVFlowVal,10,1);
            strItemAlmVal = ValueToString(g_devConfigInfo_Edt.devAlmParam.unAlmLFlowV,10,1);
            AddRLAlmItem(unRowIndex++,strAlmItem,strItemVal,strItemAlmVal);
        }
        if(g_devAlmBits.VpumGrpAlm[unGrpIndex].bBubblBits)
        {
            strAlmItem = QString("静脉血泵组<%1>---有汽包").arg(unGrpIndex+1);
            strItemVal = ValueToString(1,1,0);
