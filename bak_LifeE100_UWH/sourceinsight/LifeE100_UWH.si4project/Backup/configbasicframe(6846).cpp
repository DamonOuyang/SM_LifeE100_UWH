    ...
    ui(new Ui::ConfigBasicFrame)
{
    ...
    m_pAlmEnFlag->addItem("绂佺敤");
    m_pAlmEnFlag->addItem("鍚敤");
...
void ConfigBasicFrame::confirmString(QString strIn)
{
    if(NULL != m_pCurLineEdit)//&&m_pCurLineEdit==m_pDataTimeEdit)
    {
        ...
        if(m_pCurLineEdit==m_pDataTimeEdit)
        {
			...
			SystemEx("hwclock -w");//鏂版牳蹇冩澘锛屾柊绯荤粺浣跨敤
...
void ConfigBasicFrame::OnExitLogin(void)
{
    if(true==g_pwdCfgInfo.bPwdEnFlg)
    {
		...
		msgBox.setWindowTitle("鎻愮ず");
		msgBox.setIcon(QMessageBox::Warning);
		msgBox.setText("鏄惁瑕侀€€鍑哄瘑鐮侀獙璇?閫€鍑哄悗闇€瑕侀噸鏂伴獙璇?");
		pNewBut = msgBox.addButton(tr("纭畾"), QMessageBox::AcceptRole);
		pNewBut->setMinimumSize(100,30);
		pNewBut = msgBox.addButton(tr("鍙栨秷"), QMessageBox::RejectRole);
		pNewBut->setMinimumSize(100,30);
		SetMessageBoxFont(&msgBox);
		if(QMessageBox::AcceptRole == msgBox.exec())
		{
		    //閫€鍑哄瘑鐮侀獙璇?		    g_bLoginFlag = true;
			emit exitLoginFlg();
		}	
    }
    else
    {
        QMessageBox msgBox;
        ShowInfo(&msgBox,"瀵嗙爜杩樻病鏈夊惎鐢紝璇疯繘鍏ュ瘑鐮佽缃晫闈㈣缃悗鎵嶈兘鍚敤!","閿欒",QMessageBox::Critical);
    }
}
