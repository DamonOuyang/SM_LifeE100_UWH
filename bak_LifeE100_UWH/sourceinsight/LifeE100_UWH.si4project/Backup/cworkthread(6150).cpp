...
void CWorkThread::InitThread(void)
{
	...
	//对应板子---p4
	pUartSkt = new CUARTSocketJBD(m_pSktHandler);
#if RUN_ARM_LINUX
	if(false==pUartSkt->OpenDev("/dev/ttyAMA5",9600))
#else
	if(false==pUartSkt->OpenDev((char*)"/dev/ttyUSB2",9600))
#endif
	{
		printf("Open CommPort:%s FAIL!\r\n","/dev/ttyAMA5");
		delete pUartSkt;
...
void CWorkThread::CreateJbdCell(void)
{
    UINT8 lIndex;
    CJbdCell* pJbdCell;

    pJbdCell = new CJbdCell();//
    pJbdCell->SetDevID(0xDD);
	pJbdCell->SetSndID(MAIN_DEV_BAS_ID);
    pJbdCell->SetUARTSocket(m_pUARTSkt3);
    pJbdCell->SetWorkThread(this);
    pJbdCell->SetEnable(true);
    m_pDevCmdHander->AddSubDevice(pJbdCell);
    m_listCelMods.append(pJbdCell);
	m_procPour.AddJbdCellMod(pJbdCell);
}
...
void CWorkThread::ProcAlmInfo(void)
{
    ...
    {
		//转速传感器异常
		unSenState = m_procPour.GetSensorState(DEV_APUMP_TYPE, PUM_SPE_BITS);
		if(unSenState == 1&&\
		   g_devAlmBits.ApumGrpAlm[unIndex].bSpeeSensorBits == false)
		{
			SetAlmItem(g_devAlmBits.ApumGrpAlm[unIndex].bSpeeSensorBits,\
					 ALM_ITEM_A_SPE_SEN,unIndex+APUM_DEV_BAS_ID,0x00,\
					 unSenState,1);
			g_devAlmBits.ApumGrpAlm[unIndex].bSpeeSensorBits = true;
			bAlmFlag = true;
		}
		else if(unSenState == 0&&\
				g_devAlmBits.ApumGrpAlm[unIndex].bSpeeSensorBits == true)
		{
			SetAlmItem(g_devAlmBits.ApumGrpAlm[unIndex].bSpeeSensorBits,\
					 ALM_ITEM_A_SPE_SEN,unIndex+APUM_DEV_BAS_ID,0x00,\
		...
		//压力传感器异常
		unSenState = m_procPour.GetSensorState(DEV_APUMP_TYPE, PUM_PRE_BITS);
		if(unSenState == 1&&\
		   g_devAlmBits.ApumGrpAlm[unIndex].bPressSensorBits == false)
		{
			SetAlmItem(g_devAlmBits.ApumGrpAlm[unIndex].bPressSensorBits,\
					 ALM_ITEM_A_PRE_SEN,unIndex+APUM_DEV_BAS_ID,0x00,\
					 unSenState,1);
			g_devAlmBits.ApumGrpAlm[unIndex].bPressSensorBits = true;
			bAlmFlag = true;
		}
		else if(unSenState == 0&&\
				g_devAlmBits.ApumGrpAlm[unIndex].bPressSensorBits == true)
        ...
        if(g_devAlmBits.ApumGrpAlm[unIndex].bPressSensorBits == false)
        {
	        //泵压力高
			...
			if(g_pourTstData.tstData.unAPrssVal < 0 &&\
			    g_devAlmBits.ApumGrpAlm[unIndex].bPressLBits == false)
			{
				if(m_bBpRunFlag[0] == true && m_unDataType[0] == STR_POUR)
				{
				  SetAlmItem(g_devAlmBits.ApumGrpAlm[unIndex].bPressLBits,\
							 ALM_ITEM_A_PRE_DN,unIndex+APUM_DEV_BAS_ID,0x00,\
							 g_pourTstData.tstData.unAPrssVal,1);
				  
				  g_DataAlarm.unAPrssVal = g_pourTstData.tstData.unAPrssVal;
					  
				  SetWindowShowAlm(g_devAlmBits.ApumGrpAlm[unIndex].bPressLBits,\
							 ALM_ITEM_A_PRE_DN,unIndex+APUM_DEV_BAS_ID,0x00,\
							 g_pourTstData.tstData.unAPrssVal,1);
				  g_devAlmBits.ApumGrpAlm[unIndex].bPressLBits = 1;
				  bAlmFlag = true;
				}
			}
			else if(g_pourTstData.tstData.unAPrssVal >= MAX_ALM_PRES_DIF_DN &&\
					g_devAlmBits.ApumGrpAlm[unIndex].bPressLBits == true)
			{
			  SetAlmItem(g_devAlmBits.ApumGrpAlm[unIndex].bPressLBits,\
						 ALM_ITEM_A_PRE_DN,unIndex+APUM_DEV_BAS_ID,0x00,\
						 g_pourTstData.tstData.unAPrssVal,0);
			  ...
			  SetWindowShowAlm(g_devAlmBits.ApumGrpAlm[unIndex].bPressLBits,\
						 ALM_ITEM_A_PRE_DN,unIndex+APUM_DEV_BAS_ID,0x00,\
						 g_pourTstData.tstData.unAPrssVal,0); 
			  g_devAlmBits.ApumGrpAlm[unIndex].bPressLBits = 0;
			  //printf("eee-------unAPrssVal:%d ---unAlmUPresA:%d \r\n",g_pourTstData.tstData.unAPrssVal,g_devConfigInfo.devAlmParam.unAlmUPresA);
			}
        }	
		//流量传感器异常
		unSenState = m_procPour.GetSensorState(DEV_APUMP_TYPE, PUM_FLO_BITS);
		if(unSenState == 1&&\
		   g_devAlmBits.ApumGrpAlm[unIndex].bFlowSensorBits == false)
		{
			SetAlmItem(g_devAlmBits.ApumGrpAlm[unIndex].bFlowSensorBits,\
					 ALM_ITEM_A_FLO_SEN,unIndex+APUM_DEV_BAS_ID,0x00,\
					 unSenState,1);
			g_devAlmBits.ApumGrpAlm[unIndex].bFlowSensorBits = true;
			bAlmFlag = true;
		}
		else if(unSenState == 0&&\
				g_devAlmBits.ApumGrpAlm[unIndex].bFlowSensorBits == true)
		...
		if(g_devAlmBits.ApumGrpAlm[unIndex].bFlowSensorBits == false)
		{
	        //泵压流量高
		...
		g_devConfigInfo.devAlmParam.unAlmBubbleA)
		{
			...
			{
				SetAlmItem(g_devAlmBits.ApumGrpAlm[unIndex].bBubblBits,\
						 ALM_ITEM_A_BUBBLE,unIndex+APUM_DEV_BAS_ID,0x00,\
						 unBubbVal,1);
				g_devAlmBits.ApumGrpAlm[unIndex].bBubblBits = 1;
				bAlmFlag = true;
			}
		}
		else if(unBubbVal == 0&&\
				g_devAlmBits.ApumGrpAlm[unIndex].bBubblBits == true)
		{
			SetAlmItem(g_devAlmBits.ApumGrpAlm[unIndex].bBubblBits,\
					 ALM_ITEM_A_BUBBLE,unIndex+APUM_DEV_BAS_ID,0x00,\
					 unBubbVal,0);
			g_devAlmBits.ApumGrpAlm[unIndex].bBubblBits = 0;
		}
		//血温度传感器异常
		unSenState = m_procPour.GetSensorState(DEV_APUMP_TYPE, PUM_TMP_BITS);
		if(unSenState == 1&&\
		   g_devAlmBits.ApumGrpAlm[unIndex].bTmpSensorBits == false)
		{
			SetAlmItem(g_devAlmBits.ApumGrpAlm[unIndex].bTmpSensorBits,\
					 ALM_ITEM_A_TMP_SEN,unIndex+APUM_DEV_BAS_ID,0x00,\
					 unSenState,1);
			g_devAlmBits.ApumGrpAlm[unIndex].bTmpSensorBits = true;
			bAlmFlag = true;
		}
		else if(unSenState == 0&&\
				g_devAlmBits.ApumGrpAlm[unIndex].bTmpSensorBits == true)
	    ...
	    //血温过高
		if(g_devAlmBits.ApumGrpAlm[unIndex].bTmpSensorBits == false)
		{
		    if((g_pourTstData.tstData.unATmpVal > \
		        g_devConfigInfo.devAlmParam.unAlmUTmpA)&&\
		        g_devAlmBits.ApumGrpAlm[unIndex].bTmpUBits == false)
		    {
		        if(m_bBpRunFlag[0] == true && m_unDataType[0] == STR_POUR)
		        {
		            SetAlmItem(g_devAlmBits.ApumGrpAlm[unIndex].bTmpUBits,\
		                       ALM_ITEM_A_TMP_UP,unIndex+APUM_DEV_BAS_ID,0x00,\
		                       g_pourTstData.tstData.unATmpVal,1);
					g_DataAlarm.unATmpVal = g_pourTstData.tstData.unATmpVal;

					SetWindowShowAlm(g_devAlmBits.ApumGrpAlm[unIndex].bTmpUBits,\
		                       ALM_ITEM_A_TMP_UP,unIndex+APUM_DEV_BAS_ID,0x00,\
		                       g_pourTstData.tstData.unATmpVal,1);
					g_devAlmBits.ApumGrpAlm[unIndex].bTmpUBits = 1;
		            bAlmFlag = true;
		        }
		    }
		    else if(((g_devConfigInfo.devAlmParam.unAlmUTmpA - \
		              g_pourTstData.tstData.unATmpVal)>MAX_ALM_TMP_DIF)&&\
		              g_devAlmBits.ApumGrpAlm[unIndex].bTmpUBits == true)
		    {
		        SetAlmItem(g_devAlmBits.ApumGrpAlm[unIndex].bTmpUBits,\
		                   ALM_ITEM_A_TMP_UP,unIndex+APUM_DEV_BAS_ID,0x00,\
				...
				SetWindowShowAlm(g_devAlmBits.ApumGrpAlm[unIndex].bTmpUBits,\
		                   ALM_ITEM_A_TMP_UP,unIndex+APUM_DEV_BAS_ID,0x00,\
		                   g_pourTstData.tstData.unATmpVal,0);
		        g_devAlmBits.ApumGrpAlm[unIndex].bTmpUBits = 0;
		    }
		    //血温异常
		    if((g_pourTstData.tstData.unATmpVal+SET_MAX_ALM_TMP_DIF < \
		        g_devConfigInfo.watRooParam.unTmpVal)&&\
		        g_devAlmBits.ApumGrpAlm[unIndex].bTmpErrBits == 0)
		    {
		        if(m_bBpRunFlag[0] == true && m_unDataType[0] == STR_POUR)
		        {
		            SetAlmItem(g_devAlmBits.ApumGrpAlm[unIndex].bTmpErrBits,\
		                       ALM_ERROE_A_TMP,unIndex+APUM_DEV_BAS_ID,0x00,\
		                       g_pourTstData.tstData.unATmpVal,1);
					g_DataAlarm.unATmpVal = g_pourTstData.tstData.unATmpVal;

					SetWindowShowAlm(g_devAlmBits.ApumGrpAlm[unIndex].bTmpErrBits,\
		                       ALM_ERROE_A_TMP,unIndex+APUM_DEV_BAS_ID,0x00,\
		                       g_pourTstData.tstData.unATmpVal,1);
					g_devAlmBits.ApumGrpAlm[unIndex].bTmpErrBits = 1;
		            bAlmFlag = true;
		        }
		    }
		    else if((g_pourTstData.tstData.unATmpVal+SET_MAX_ALM_TMP_DIF >= \
		             g_devConfigInfo.watRooParam.unTmpVal)&&\
		              g_devAlmBits.ApumGrpAlm[unIndex].bTmpErrBits == 1)
		    {
		        SetAlmItem(g_devAlmBits.ApumGrpAlm[unIndex].bTmpErrBits,\
    ...
    {
        //静脉转速传感器异常
		unSenState = m_procPour.GetSensorState(DEV_VPUMP_TYPE, PUM_SPE_BITS);
		if(unSenState == 1&&\
		   g_devAlmBits.VpumGrpAlm[unIndex].bSpeeSensorBits == false)
		{
			SetAlmItem(g_devAlmBits.VpumGrpAlm[unIndex].bSpeeSensorBits,\
					 ALM_ITEM_V_SPE_SEN,unIndex+VPUM_DEV_BAS_ID,0x00,\
					 unSenState,1);
			g_devAlmBits.VpumGrpAlm[unIndex].bSpeeSensorBits = true;
			bAlmFlag = true;
		}
		else if(unSenState == 0&&\
				g_devAlmBits.VpumGrpAlm[unIndex].bSpeeSensorBits == true)
        ...
        //静脉压力传感器异常
		unSenState = m_procPour.GetSensorState(DEV_VPUMP_TYPE, PUM_PRE_BITS);
		if(unSenState == 1&&\
		   g_devAlmBits.VpumGrpAlm[unIndex].bPressSensorBits == false)
		{
			SetAlmItem(g_devAlmBits.VpumGrpAlm[unIndex].bPressSensorBits,\
					 ALM_ITEM_V_PRE_SEN,unIndex+VPUM_DEV_BAS_ID,0x00,\
					 unSenState,1);
			g_devAlmBits.VpumGrpAlm[unIndex].bPressSensorBits = true;
			bAlmFlag = true;
		}
		else if(unSenState == 0&&\
				g_devAlmBits.VpumGrpAlm[unIndex].bPressSensorBits == true)
        ...
        if(g_devAlmBits.VpumGrpAlm[unIndex].bPressSensorBits == false)
        {
	        //泵压力高
	        if((g_pourTstData.tstData.unVPrssVal >= \
	            g_devConfigInfo.devAlmParam.unAlmUPresV)&&\
	            g_devAlmBits.VpumGrpAlm[unIndex].bPressUBits == false)
	        {
	            if(m_bBpRunFlag[1] == true && m_unDataType[1] == STR_POUR)
	            {
		            SetAlmItem(g_devAlmBits.VpumGrpAlm[unIndex].bPressUBits,\
		                       ALM_ITEM_V_PRE_UP,unIndex+VPUM_DEV_BAS_ID,0x00,\
		                       g_pourTstData.tstData.unVPrssVal,1);
					
	                g_DataAlarm.unVPrssVal = g_pourTstData.tstData.unVPrssVal;
						
		            SetWindowShowAlm(g_devAlmBits.VpumGrpAlm[unIndex].bPressUBits,\
		                       ALM_ITEM_V_PRE_UP,unIndex+VPUM_DEV_BAS_ID,0x00,\
		                       g_pourTstData.tstData.unVPrssVal,1);
					g_devAlmBits.VpumGrpAlm[unIndex].bPressUBits = 1;
		            bAlmFlag = true;
	            }
				//printf("sss-------unAPrssVal:%d ---unAlmUPresA:%d \r\n",g_pourTstData.tstData.unAPrssVal,g_devConfigInfo.devAlmParam.unAlmUPresA);
	        }
	        else if(((g_devConfigInfo.devAlmParam.unAlmUPresV - \
	                  g_pourTstData.tstData.unVPrssVal)>V_MAX_ALM_PRES_DIF)&&\
	                  g_devAlmBits.VpumGrpAlm[unIndex].bPressUBits == true)
	        {
	            SetAlmItem(g_devAlmBits.VpumGrpAlm[unIndex].bPressUBits,\
	                       ALM_ITEM_V_PRE_UP,unIndex+VPUM_DEV_BAS_ID,0x00,\
	            ...
	            SetWindowShowAlm(g_devAlmBits.VpumGrpAlm[unIndex].bPressUBits,\
	                       ALM_ITEM_V_PRE_UP,unIndex+VPUM_DEV_BAS_ID,0x00,\
	                       g_pourTstData.tstData.unVPrssVal,0);	
				g_devAlmBits.VpumGrpAlm[unIndex].bPressUBits = 0;
				//printf("eee-------unAPrssVal:%d ---unAlmUPresA:%d \r\n",g_pourTstData.tstData.unAPrssVal,g_devConfigInfo.devAlmParam.unAlmUPresA);
	        }
			//泵压力低
			if(g_pourTstData.tstData.unVPrssVal < 0&&\
			   g_devAlmBits.VpumGrpAlm[unIndex].bPressLBits == false)
			{
				if(m_bBpRunFlag[1] == true && m_unDataType[1] == STR_POUR)
				{
				  SetAlmItem(g_devAlmBits.VpumGrpAlm[unIndex].bPressLBits,\
							 ALM_ITEM_V_PRE_DN,unIndex+VPUM_DEV_BAS_ID,0x00,\
							 g_pourTstData.tstData.unVPrssVal,1);
				  
				  g_DataAlarm.unVPrssVal = g_pourTstData.tstData.unVPrssVal;
					  
				  SetWindowShowAlm(g_devAlmBits.VpumGrpAlm[unIndex].bPressLBits,\
							 ALM_ITEM_V_PRE_DN,unIndex+VPUM_DEV_BAS_ID,0x00,\
							 g_pourTstData.tstData.unVPrssVal,1);
				  g_devAlmBits.VpumGrpAlm[unIndex].bPressLBits = 1;
				  bAlmFlag = true;
				}
			}
			else if(g_pourTstData.tstData.unVPrssVal >= V_MAX_ALM_PRES_DIF_DN &&\
					g_devAlmBits.VpumGrpAlm[unIndex].bPressLBits == true)
			{
			  SetAlmItem(g_devAlmBits.VpumGrpAlm[unIndex].bPressLBits,\
						 ALM_ITEM_V_PRE_DN,unIndex+VPUM_DEV_BAS_ID,0x00,\
						 g_pourTstData.tstData.unVPrssVal,0);
			  ...
			  SetWindowShowAlm(g_devAlmBits.VpumGrpAlm[unIndex].bPressLBits,\
						 ALM_ITEM_V_PRE_DN,unIndex+VPUM_DEV_BAS_ID,0x00,\
						 g_pourTstData.tstData.unVPrssVal,0); 
			  g_devAlmBits.VpumGrpAlm[unIndex].bPressLBits = 0;
			}
        }
        //静脉流量传感器异常
		unSenState = m_procPour.GetSensorState(DEV_VPUMP_TYPE, PUM_FLO_BITS);
		if(unSenState == 1&&\
		   g_devAlmBits.VpumGrpAlm[unIndex].bFlowSensorBits == false)
		{
			SetAlmItem(g_devAlmBits.VpumGrpAlm[unIndex].bFlowSensorBits,\
					 ALM_ITEM_V_FLO_SEN,unIndex+VPUM_DEV_BAS_ID,0x00,\
					 unSenState,1);
			g_devAlmBits.VpumGrpAlm[unIndex].bFlowSensorBits = true;
			bAlmFlag = true;
		}
		else if(unSenState == 0&&\
				g_devAlmBits.VpumGrpAlm[unIndex].bFlowSensorBits == true)
		...
		if(g_devAlmBits.VpumGrpAlm[unIndex].bFlowSensorBits == false)
		{
	        //泵压流量高
	        if((g_pourTstData.tstData.unVFlowVal >= \
	            g_devConfigInfo.devAlmParam.unAlmUFlowV)&&\
	            g_devAlmBits.VpumGrpAlm[unIndex].bFlowUBits == false)
	        {
	            if(m_bBpRunFlag[1] == true && m_unDataType[1] == STR_POUR)
	            {
		            SetAlmItem(g_devAlmBits.VpumGrpAlm[unIndex].bFlowUBits,\
		                       ALM_ITEM_V_FLO_UP,unIndex+VPUM_DEV_BAS_ID,0x00,\
		                       g_pourTstData.tstData.unVFlowVal,1);
					g_DataAlarm.unVFlowVal = g_pourTstData.tstData.unVFlowVal;

					SetWindowShowAlm(g_devAlmBits.VpumGrpAlm[unIndex].bFlowUBits,\
		                       ALM_ITEM_V_FLO_UP,unIndex+VPUM_DEV_BAS_ID,0x00,\
		                       g_pourTstData.tstData.unVFlowVal,1);
					g_devAlmBits.VpumGrpAlm[unIndex].bFlowUBits = 1;
		            bAlmFlag = true;
	            }
	        }
	        else if(((g_devConfigInfo.devAlmParam.unAlmUFlowV - \
	                  g_pourTstData.tstData.unVFlowVal)>MAX_ALM_FLOW_DIF)&&\
	                  g_devAlmBits.VpumGrpAlm[unIndex].bFlowUBits == true)
	        {
	            SetAlmItem(g_devAlmBits.VpumGrpAlm[unIndex].bFlowUBits,\
	                       ALM_ITEM_V_FLO_UP,unIndex+VPUM_DEV_BAS_ID,0x00,\
				...
				SetWindowShowAlm(g_devAlmBits.VpumGrpAlm[unIndex].bFlowUBits,\
	                       ALM_ITEM_V_FLO_UP,unIndex+VPUM_DEV_BAS_ID,0x00,\
	                       g_pourTstData.tstData.unVFlowVal,0);
	            g_devAlmBits.VpumGrpAlm[unIndex].bFlowUBits = 0;
	        }
	        //泵压流量低
	        if((g_pourTstData.tstData.unVFlowVal < \
	            g_devConfigInfo.devAlmParam.unAlmLFlowV)&&\
	            g_devAlmBits.VpumGrpAlm[unIndex].bFlowLBits == false)
	        {
	            //流量告警 泵开，等待5秒
	            if(m_bBpRunFlag[1] == true  && m_unDataType[1] == STR_POUR && \
				   m_unAlmFlowDlySec[1] > 5)
	            {
		            SetAlmItem(g_devAlmBits.VpumGrpAlm[unIndex].bFlowLBits,\
		                       ALM_ITEM_V_FLO_DN,unIndex+VPUM_DEV_BAS_ID,0x00,\
		                       g_pourTstData.tstData.unVFlowVal,1);
					g_DataAlarm.unVFlowVal = g_pourTstData.tstData.unVFlowVal;

					SetWindowShowAlm(g_devAlmBits.VpumGrpAlm[unIndex].bFlowLBits,\
		                       ALM_ITEM_V_FLO_DN,unIndex+VPUM_DEV_BAS_ID,0x00,\
		                       g_pourTstData.tstData.unVFlowVal,1);
					g_devAlmBits.VpumGrpAlm[unIndex].bFlowLBits = 1;
		            bAlmFlag = true;
	            }
	        }
	        else if(((g_pourTstData.tstData.unVFlowVal - \
	                  g_devConfigInfo.devAlmParam.unAlmLFlowV)>MAX_ALM_FLOW_DIF)&&\
	                  g_devAlmBits.VpumGrpAlm[unIndex].bFlowLBits == true)
	        {
	            SetAlmItem(g_devAlmBits.VpumGrpAlm[unIndex].bFlowLBits,\
	                       ALM_ITEM_V_FLO_DN,unIndex+VPUM_DEV_BAS_ID,0x00,\
	                       g_pourTstData.tstData.unVFlowVal,0);
				...
				g_devAlmBits.VpumGrpAlm[unIndex].bFlowLBits = 0;
	        }
		}
		//血泵有气泡
		...
		{
			//气泡告警 泵开，等待5秒
			if(m_bBpRunFlag[1] == true && m_unAlmFlowDlySec[1] > 5)
			{
				SetAlmItem(g_devAlmBits.VpumGrpAlm[unIndex].bBubblBits,\
						 ALM_ITEM_V_BUBBLE,unIndex+VPUM_DEV_BAS_ID,0x00,\
						 unBubbVal,1);
				g_devAlmBits.VpumGrpAlm[unIndex].bBubblBits = 1;
				bAlmFlag = true;
			}
		}
		else if(unBubbVal == 0&&\
				g_devAlmBits.VpumGrpAlm[unIndex].bBubblBits == true)
		{
			SetAlmItem(g_devAlmBits.VpumGrpAlm[unIndex].bBubblBits,\
					 ALM_ITEM_V_BUBBLE,unIndex+VPUM_DEV_BAS_ID,0x00,\
					 unBubbVal,0);
			g_devAlmBits.VpumGrpAlm[unIndex].bBubblBits = 0;
		}
		//静脉血温传感器异常
		unSenState = m_procPour.GetSensorState(DEV_VPUMP_TYPE, PUM_TMP_BITS);
		if(unSenState == 1&&\
		   g_devAlmBits.VpumGrpAlm[unIndex].bTmpSensorBits == false)
		{
			SetAlmItem(g_devAlmBits.VpumGrpAlm[unIndex].bTmpSensorBits,\
					 ALM_ITEM_V_TMP_SEN,unIndex+VPUM_DEV_BAS_ID,0x00,\
					 unSenState,1);
			g_devAlmBits.VpumGrpAlm[unIndex].bTmpSensorBits = true;
			bAlmFlag = true;
		}
		else if(unSenState == 0&&\
				g_devAlmBits.VpumGrpAlm[unIndex].bTmpSensorBits == true)
        ...
        if(g_devAlmBits.VpumGrpAlm[unIndex].bTmpSensorBits == false)
        {
		    //血温过高
		    if((g_pourTstData.tstData.unVTmpVal > \
		        g_devConfigInfo.devAlmParam.unAlmUTmpV)&&\
		        g_devAlmBits.VpumGrpAlm[unIndex].bTmpUBits == false)
		    {
		        if(m_bBpRunFlag[1] == true && m_unDataType[1] == STR_POUR)
		        {
		            SetAlmItem(g_devAlmBits.VpumGrpAlm[unIndex].bTmpUBits,\
		                       ALM_ITEM_V_TMP_UP,unIndex+VPUM_DEV_BAS_ID,0x00,\
		                       g_pourTstData.tstData.unVTmpVal,1);
					g_DataAlarm.unVTmpVal = g_pourTstData.tstData.unVTmpVal;

					SetWindowShowAlm(g_devAlmBits.VpumGrpAlm[unIndex].bTmpUBits,\
		                       ALM_ITEM_V_TMP_UP,unIndex+VPUM_DEV_BAS_ID,0x00,\
		                       g_pourTstData.tstData.unVTmpVal,1);
					g_devAlmBits.VpumGrpAlm[unIndex].bTmpUBits = 1;
		            bAlmFlag = true;
		        }
		    }
		    else if(((g_devConfigInfo.devAlmParam.unAlmUTmpV - \
		              g_pourTstData.tstData.unVTmpVal)>MAX_ALM_TMP_DIF)&&\
		              g_devAlmBits.VpumGrpAlm[unIndex].bTmpUBits == true)
		    {
		        SetAlmItem(g_devAlmBits.VpumGrpAlm[unIndex].bTmpUBits,\
		                   ALM_ITEM_V_TMP_UP,unIndex+VPUM_DEV_BAS_ID,0x00,\
				...
				SetWindowShowAlm(g_devAlmBits.VpumGrpAlm[unIndex].bTmpUBits,\
		                   ALM_ITEM_V_TMP_UP,unIndex+VPUM_DEV_BAS_ID,0x00,\
		                   g_pourTstData.tstData.unVTmpVal,0);
		        g_devAlmBits.VpumGrpAlm[unIndex].bTmpUBits = 0;
		    }	
		    //血温异常
		    if((g_pourTstData.tstData.unVTmpVal+SET_MAX_ALM_TMP_DIF < \
		        g_devConfigInfo.watRooParam.unTmpVal)&&\
		        g_devAlmBits.VpumGrpAlm[unIndex].bTmpErrBits == false)
		    {
		        if(m_bBpRunFlag[1] == true && m_unDataType[1] == STR_POUR)
		        {
		            SetAlmItem(g_devAlmBits.VpumGrpAlm[unIndex].bTmpErrBits,\
		                       ALM_ERROE_V_TMP,unIndex+VPUM_DEV_BAS_ID,0x00,\
		                       g_pourTstData.tstData.unVTmpVal,1);
					g_DataAlarm.unVTmpVal = g_pourTstData.tstData.unVTmpVal;

					SetWindowShowAlm(g_devAlmBits.VpumGrpAlm[unIndex].bTmpErrBits,\
		                       ALM_ERROE_V_TMP,unIndex+VPUM_DEV_BAS_ID,0x00,\
		                       g_pourTstData.tstData.unVTmpVal,1);
					g_devAlmBits.VpumGrpAlm[unIndex].bTmpErrBits = 1;
		            bAlmFlag = true;
		        }
		    }
		    else if((g_pourTstData.tstData.unVTmpVal+SET_MAX_ALM_TMP_DIF >= \
		             g_devConfigInfo.watRooParam.unTmpVal)&&\
		              g_devAlmBits.VpumGrpAlm[unIndex].bTmpErrBits == true)
		    {
		        SetAlmItem(g_devAlmBits.VpumGrpAlm[unIndex].bTmpErrBits,\
		                   ALM_ERROE_V_TMP,unIndex+VPUM_DEV_BAS_ID,0x00,\
		                   g_pourTstData.tstData.unVTmpVal,0);
				...
				SetWindowShowAlm(g_devAlmBits.VpumGrpAlm[unIndex].bTmpErrBits,\
		                   ALM_ERROE_V_TMP,unIndex+VPUM_DEV_BAS_ID,0x00,\
		                   g_pourTstData.tstData.unVTmpVal,0);
		        g_devAlmBits.VpumGrpAlm[unIndex].bTmpErrBits = 0;
		    }
        }
	}
	//水浴
    ...
    {
		//水浴水温传感器异常
		unSenState = m_procPour.GetSensorState(DEV_WAT_TYPE, WAT_TMP_BITS);
		if(unSenState == 1&&\
		   g_devAlmBits.watGrpAlm[unIndex].bTmpSensorBits == false)
		{
			SetAlmItem(g_devAlmBits.watGrpAlm[unIndex].bTmpSensorBits,\
					 ALM_ITEM_WAT_TMP_SEN,unIndex+WAT_DEV_BAS_ID,0x00,\
					 unSenState,1);
			g_devAlmBits.watGrpAlm[unIndex].bTmpSensorBits = true;
			bAlmFlag = true;
		}
		else if(unSenState == 0&&\
				g_devAlmBits.watGrpAlm[unIndex].bTmpSensorBits == true)
		...
		if(g_devAlmBits.watGrpAlm[unIndex].bTmpSensorBits == false)
		{
	        //水浴---温度
		...
		/*
		unSenState = m_procPour.GetSensorState(DEV_WAT_TYPE, WAT_LIN_BITS);
		if(unSenState == 1&&\
		   g_devAlmBits.watGrpAlm[unIndex].bLinSensorBits == false)
		{
			SetAlmItem(g_devAlmBits.watGrpAlm[unIndex].bLinSensorBits,\
					 ALM_ITEM_WAT_LIN_SEN,unIndex+WAT_DEV_BAS_ID,0x00,\
					 unSenState,1);
			g_devAlmBits.watGrpAlm[unIndex].bLinSensorBits = true;
			bAlmFlag = true;
		}
		else if(unSenState == 0&&\
				g_devAlmBits.watGrpAlm[unIndex].bLinSensorBits == true)
...
*/
	  	        ...
	  	        g_devAlmBits.watGrpAlm[unIndex].bLinLBits == true)
	  {
		  ...
		  g_devAlmBits.watGrpAlm[unIndex].bLinLBits = 0;
				...
				/*
		unSenState = m_procPour.GetSensorState(DEV_WAT_TYPE, WAT_BLO_BITS);
		if(unSenState == 1&&\
		   g_devAlmBits.watGrpAlm[unIndex].bBloSensorBits == false)
		{
			SetAlmItem(g_devAlmBits.watGrpAlm[unIndex].bBloSensorBits,\
					 ALM_ITEM_WAT_BLO_SEN,unIndex+WAT_DEV_BAS_ID,0x00,\
					 unSenState,1);
			g_devAlmBits.watGrpAlm[unIndex].bBloSensorBits = true;
			bAlmFlag = true;
		}
		else if(unSenState == 0&&\
				g_devAlmBits.watGrpAlm[unIndex].bBloSensorBits == true)
