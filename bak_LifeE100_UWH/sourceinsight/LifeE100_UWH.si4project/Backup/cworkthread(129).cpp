...
void CWorkThread::SetPumState(UINT8     unPumType)
{
    OperLogCtl operLogCtl;
	CSwitPump* pSwitPum;
	INT16 unPumPrm = 0;
	...
	if(unPumType == DEV_APUMP_TYPE)//动脉泵
	{
		...
		{
		    unPumPrm = g_devConfigInfo_Edt.arteryParm.unSpeed;
		}
		else
		{
		    unPumPrm = g_devConfigInfo_Edt.arteryParm.unPress;
		}
	...
	else if(unPumType == DEV_VPUMP_TYPE)
	{
		...
		if(m_unDataType[unIndex] == PRE_POUR)
		{
		    unPumPrm = g_devConfigInfo_Edt.veinParm.unSpeed;
		}
		else
		{
		    unPumPrm = g_devConfigInfo_Edt.veinParm.unPress;
		}
	...
	operLogCtl.unOperType[0] = 0;//停止
	operLogCtl.unOperType[1] = 1;//启动
	SaveOperLogData((const char*)&operLogCtl, sizeof(OperLogCtl));
	SaveOperLogInfo();	
	
	SetPumSta pPumSta;
	pPumSta.unPumSat = m_bBpRunFlag[unIndex];
	pPumSta.unPourSta = m_unDataType[unIndex];
	pPumSta.unPumVal = unPumPrm;
	pSwitPum->SetPumpState(&pPumSta);
}
...
void CWorkThread::ProcAlmInfo(void)
{
    ...
    if((g_pourTstData.tstData.unATmpVal+SET_MAX_ALM_TMP_DIF < \
        g_devConfigInfo.watRooParam.unTmpVal)&&\
        g_devAlmBits.ApumGrpAlm[unIndex].bTmpErrBits == false)
    {
        if(m_bBpRunFlag[0] == true && m_unDataType[0] == STR_POUR)
        {
            ...
            bAlmFlag = true;
             ...
             g_devConfigInfo.watRooParam.unTmpVal)&&\
              g_devAlmBits.ApumGrpAlm[unIndex].bTmpErrBits == true)
    {
        SetAlmItem(g_devAlmBits.ApumGrpAlm[unIndex].bTmpErrBits,\
                   ALM_ERROE_A_TMP,unIndex+APUM_DEV_BAS_ID,0x00,\
                   g_pourTstData.tstData.unATmpVal,0);
    ...
    {
        SetAlmItem(g_devAlmBits.VpumGrpAlm[unIndex].bTmpUBits,\
                   ALM_ITEM_V_TMP_UP,unIndex+APUM_DEV_BAS_ID,0x00,\
