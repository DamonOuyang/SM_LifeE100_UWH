...
public:
	...
	void AddJbdCellMod(CJbdCell* pCllModule);
	void RmvJbdCellMod(CJbdCell* pCllModule);
	void AddBq40Z50Mod(CBq40z50Bat* pCllModule);
	void RmvBq40Z50Mod(CBq40z50Bat* pCllModule);
	...
	UINT32 GetBatBascData(UINT8 unIndex, BYTE* pDataBuf,UINT32& unBufSize);
	bool GetBatLink(void);
    void GetMucVerInfo(BYTE* pDataBuf);
	UINT8 GetWatBloodState(void);
	UINT8 GetBubbleVal(UINT8 unPunType);
	UINT8 GetDevAlmState(UINT8 unPunType, UINT32 unSenType);
	UINT8 GetBatterySoc(void);
	UINT16 GetBatteryTmp(void);
	UINT8 GetBatteryLos(void);
protected:
    QList<CSwitPump*>m_listAPumModules;
	QList<CSwitPump*>m_listVPumModules;
	QList<CSwitWater*>m_listWatModules;
	//QList<CJbdCell*>m_listCllModules;
	QList<CBq40z50Bat*>m_listCllModules;

	PumpDevData m_pumADevData;//动脉泵
	PumpDevData m_pumVDevData;//静脉泵
	WatDevData m_watDevData;
	//BatBascInfo m_batDevData;//电池
	BqBatBascInfo m_batDevData;//电池
