...
CPourDevices::CPourDevices()
{
	...
	memset(&m_batDevData,0,sizeof(m_batDevData));
}
CPourDevices::~CPourDevices()
{
    DeleteAllMods();
...
//添加电池管理设备类进列表
void CPourDevices::AddBq40Z50Mod(CBq40z50Bat* pCllModule)
{
    int nItem = 0;
    while(nItem < m_listCllModules.size())
    {
        if(m_listCllModules.at(nItem)==pCllModule)
        {
            return ;
        }
        nItem++;
    }
    m_listCllModules.append(pCllModule);
}

//从列表将电池管理设备类移除
void CPourDevices::RmvBq40Z50Mod(CBq40z50Bat* pCllModule)
{
    int nItem = 0;
    while(nItem < m_listCllModules.size())
    {
        if(m_listCllModules.at(nItem)==pCllModule)
        {
            m_listCllModules.removeAt(nItem);
            return;
        }
        nItem++;
    }
...
void CPourDevices::DeleteAllMods(void)
{
    ...
    //电池
    while(unIndex < m_listCllModules.count())
    {
        pCllMod = m_listCllModules.at(unIndex++);
        delete pCllMod;
    }
    m_listCllModules.clear();
...
void CPourDevices::OnMsTimer(UINT32 unMsCount)
{
	...
	m_unBakDlyMs   = DoMsTimer(m_unBakDlyMs,unMsCount);
...
void CPourDevices::OnUpdData(bool bUpdFlag)
{
    ...
    g_pourTstData.tstData.unVFlowVal = 0xffff;//m_pumVDevData.unFlowVal;	//流量(ml)
...
void CPourDevices::TaskDevCalZero(     void)
{
    if(m_bCalTaskFlg == false)
    {
		for(UINT8 unDev = 0; unDev < 3; unDev++)
		{
		    if(g_setReadCalPrm[unDev].calPrmFlag == CAL_SET)
		    {
		        m_unCalDevIndex = unDev+1;
				m_bCalTaskFlg = true;
				printf("-------------------------------dev:%d---------------\r\n",unDev);
				break;
		    }
    ...
    if(m_unCalDevIndex == DEV_APUMP_TYPE)
    {
	    if(m_listAPumModules.size()>0)
    		pPumMod = m_listAPumModules.at(0);
		else
			return ;
    }
    else if(m_unCalDevIndex == DEV_VPUMP_TYPE)
    {
        if(m_listVPumModules.size()>0)
    		pPumMod = m_listVPumModules.at(0);
		else
			return ;
    }
    else if(m_unCalDevIndex == DEV_WAT_TYPE)
    {
        if(m_listWatModules.size()>0)
    		pPumMod = m_listWatModules.at(0);
		else
			return ;
    }
	if(false==pPumMod->IsActive())
	{
        m_strErrorInfo = QString("设备ID为<%1>:通信超时!\r\n").arg(pPumMod->GetDevID());
        printf(m_strErrorInfo.toStdString().c_str());
		g_setReadCalPrm[m_unCalDevIndex-1].calPrmFlag = 0;
		m_unCalDevIndex = 0;
		m_bCalTaskFlg = false;
		
        return ;	    
	}
	if(g_setReadCalPrm[m_unCalDevIndex-1].calPrmFlag == CAL_SET)
	{
    	pPumMod->SetCalZeroCmd(g_setReadCalPrm[m_unCalDevIndex-1].bstCalPrm.bCalItem);
		g_setReadCalPrm[m_unCalDevIndex-1].calPrmFlag = CAL_SET_SND;
		m_unBakDlyMs = 10*1000L;	
	}
	else if(g_setReadCalPrm[m_unCalDevIndex-1].calPrmFlag == CAL_SET_SND)
	{
	    UINT8 unCalBak = pPumMod->GetCalZreoBak();
	    if(unCalBak > 0)
	    {
	        g_setReadCalPrm[m_unCalDevIndex-1].calPrmFlag = unCalBak;
			m_unCalDevIndex = 0;
			m_bCalTaskFlg = false;
	    }
		else if(m_unBakDlyMs == 0)
		{
		    g_setReadCalPrm[m_unCalDevIndex-1].calPrmFlag = CAL_SET_FAL;
			m_unCalDevIndex = 0;
			m_bCalTaskFlg = false;
		}
	}
...
void CPourDevices::TaskSaveErrInfo(void)
{
    char pGetErrBuf[254];
    if(m_listAPumModules.size() > 0)
    {
		CSwitPump* pSwitPum;
		pSwitPum = m_listAPumModules.at(0);
		if(pSwitPum->GetErrInfiFlag())
		{
		    //读取信息
		    memset(&pGetErrBuf,0,sizeof(pGetErrBuf));
		    pSwitPum->GetErrInfo(pGetErrBuf);
		}
    }
    if(m_listVPumModules.size() > 0)
    {
		CSwitPump* pSwitPum;
		pSwitPum = m_listVPumModules.at(0);
		if(pSwitPum->GetErrInfiFlag())
		{
		    //读取信息
		    memset(&pGetErrBuf,0,sizeof(pGetErrBuf));
    ...
    {
		CSwitWater* pSwitPum;
		pSwitPum = m_listWatModules.at(0);
		if(pSwitPum->GetErrInfiFlag())
		{
		    //读取信息
			memset(&pGetErrBuf,0,sizeof(pGetErrBuf));
...
UINT32 CPourDevices::GetBatBascData(UINT8 unIndex, BYTE* pDataBuf,UINT32& unBufSize)
{
	//CJbdCell* pBatMod;
	CBq40z50Bat* pBatMod;
	QString strClsName;
    ...
    pBatMod = m_listCllModules.at(unIndex);
	pBatMod->GetClassName(strClsName);
	//if("CJbdCell"==strClsName)
	if("CBq40z50Bat"==strClsName)
	{
		pBatMod->GetCtrlDevData(&m_batDevData);
		unBufSize = sizeof(m_batDevData);
		memcpy((BYTE*)pDataBuf,(BYTE*)&m_batDevData,unBufSize);
	}
    unDataPos = unBufSize;
    return unDataPos;
}

bool CPourDevices::GetBatLink(void)
{
	//CJbdCell* pBatMod;
	CBq40z50Bat* pBatMod;
	QString strClsName;
    bool bBatLink;
    pBatMod = m_listCllModules.at(0);
	pBatMod->GetClassName(strClsName);
	//if("CJbdCell"==strClsName)
	if("CBq40z50Bat"==strClsName)
	{
		bBatLink = pBatMod->IsRun();
...
UINT8 CPourDevices::GetDevAlmState(UINT8 unPunType, UINT32 unSenType)
{
    UINT8 unSenSta = 0;
	
    if(unPunType == DEV_APUMP_TYPE)
    {
		if(m_pumADevData.unDevCode&unSenType)
			unSenSta = 1;
    }
	else if(unPunType == DEV_VPUMP_TYPE)
	{
		if(m_pumVDevData.unDevCode&unSenType)
			unSenSta = 1;
	}
	else if(unPunType == DEV_WAT_TYPE)
	{
		if(m_watDevData.unDevCode&unSenType)
			unSenSta = 1;
	}
...
UINT8 CPourDevices::GetBatterySoc(void)
{
	return m_batDevData.unBatSoc;
}
UINT16 CPourDevices::GetBatteryTmp(void)
{
	return m_batDevData.unBatTmp;
}
UINT8 CPourDevices::GetBatteryLos(void)
{
    UINT8 unAcLos = 0;
	if(m_batDevData.unBatCur < 0)
	{
		unAcLos = 1;
	}
	return unAcLos;
