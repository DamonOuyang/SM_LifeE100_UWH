...
void CUARTSocketJBD::SndCmdPkt(BYTE* pDataBuf,UINT32 unDataLen,UINT32 unDevID)
{
    //执行服务器发来的命令;
   UINT16 unCRC = 0;
...
void CUARTSocketJBD::SendCmdData(void)
{
    UINT32 unDataLen;
	UINT16 unCRC = 0;
    JbdPacketHead*  pUartPktHead = (JbdPacketHead*)m_pCmdBuf;
    pUartPktHead->unCmdID 	= m_cmdData.unCmdID;
    pUartPktHead->unStrID 	= m_cmdData.unDevID;
	pUartPktHead->unCmdDatLen = m_cmdData.unCmdDataLen;
    ...
    unDataLen = sizeof(JbdPacketHead)+m_cmdData.unCmdDataLen;
	unCRC = ntohs(CreateJBDCheckSum(m_pCmdBuf,unDataLen));
	unDataLen+=2;
	m_pCmdBuf[unDataLen-2] = (unCRC >> 8) & 0xFF;
	m_pCmdBuf[unDataLen-1] = unCRC & 0xFF;
