...
void AlmShowDlg::UpDateRTAlmInfos(void)
{
    ...
    for(unIndex = 0; unIndex < g_devConfigInfo.basicCfgInfo.unWatCount; unIndex++)
    {
        ...
        if(g_devAlmBits.watGrpAlm[unGrpIndex].bTmpSensorBits)
        {
            strAlmItem = QString("水浴模块<%1>---水温传感器异常").arg(unGrpIndex+1);
            strItemVal = ValueToString(1,1,0);
            strItemAlmVal = ValueToString(1,1,0);
            AddRLAlmItem(unRowIndex++,strAlmItem,strItemVal,strItemAlmVal);
        }
        if(g_devAlmBits.watGrpAlm[unGrpIndex].bLinSensorBits)
        {
            strAlmItem = QString("水浴模块<%1>---水位传感器异常").arg(unGrpIndex+1);
            strItemVal = ValueToString(1,1,0);
            strItemAlmVal = ValueToString(1,1,0);
            AddRLAlmItem(unRowIndex++,strAlmItem,strItemVal,strItemAlmVal);
        }
        if(g_devAlmBits.watGrpAlm[unGrpIndex].bBloSensorBits)
        {
            strAlmItem = QString("水浴模块<%1>---血位传感器异常").arg(unGrpIndex+1);
            strItemVal = ValueToString(1,1,0);
            strItemAlmVal = ValueToString(1,1,0);
            AddRLAlmItem(unRowIndex++,strAlmItem,strItemVal,strItemAlmVal);
        }		
