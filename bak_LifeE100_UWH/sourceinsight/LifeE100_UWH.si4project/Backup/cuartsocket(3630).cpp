...
{
    m_lckIsCmdEmpty.lock();
    bool bGetNextCmd = m_pCmdDataQueue->GetCmdData(&m_cmdData);
	m_lckIsCmdEmpty.unlock();
    if(true == bGetNextCmd)
    {
        SendCmdData();
		printf("-----unDevID:%d-----------entr CUARTSocket::SendNextCmd \r\n",m_cmdData.unDevID);
    }
}
...
void CUARTSocket::AddCmdData(CmdData* pCmdData)
{
    ...
    m_lckIsCmdEmpty.unlock();
	printf("ID:%d CUARTSocket::SendNextCmd------IsEmpty:%d\r\n",pCmdData->unDevID,m_pCmdDataQueue->IsEmpty());
}
