    ...
    ui(new Ui::MainWindow)
{
    ...
    g_pWorkThread->start();
	g_pourTstData.tstData.unDataType = 0; //启动默认预灌注
	...
	g_pourTstData.tstData.unDataType = 0;
...
void MainWindow::OnMonTabChanged(void)
{
	...
	g_pourTstData.tstData.unDataType = ui->monTabWidget->currentIndex();
	g_pWorkThread->SetDataType(DEV_APUMP_TYPE, g_pourTstData.tstData.unDataType);
	if(m_bPourStrFlg == false)
	{
	    if(g_pourTstData.tstData.unDataType == STR_POUR)
	    {
	        m_bPourStrFlg = true;
	    }
	}
...
void MainWindow::SysAutoChangePress(UINT16 unPress)
{
    printf("Entr MainWindow::SysAutoChangePress \r\n");
...
void MainWindow::CheckAutoPress(void)
{
    ...
    if(g_devAlmBits.watGrpAlm[0].bBloodBit == 1 && \
	   g_devConfigInfo_Edt.arteryParm.unPress != SET_AUTO_PRESS)
    {
        SysAutoChangePress(SET_AUTO_PRESS);
    }
}
...
void MainWindow::on_BPStrBtn_clicked(bool checked)
{
	...
	if(checked == 1)
	{
		...
		g_pWorkThread->StartBloodPump(0);
		m_bBPRunFlg = true;
...
{  
	if(false==g_pWorkThread->IsPumpRuning() && \
	   true == m_bBPRunFlg)
	{
