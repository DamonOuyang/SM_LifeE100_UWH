    ...
    QFrame(parent)
{
	...
	pFirstRow->addLayout(pFrmLayout);
	pFrmLayout = new QFormLayout;
	AddItem(pFrmLayout,m_pPourUSpeedV,tr("压力上限:"),tr("(r/min)"),lEditWidth);
	pFirstRow->addLayout(pFrmLayout);
	formLayout->addRow(pFirstRow);
...
void PourConfigPanel::OnClickOK(void)
{
    ...
    //水温保护值比告警值大1
	if(g_devConfigInfo_Edt.devPourParam.unPourUWatTmp < (g_devConfigInfo.devAlmParam.unAlmUPresA+10))
	{
    ...
    if(NULL!=m_pCurLineEdit)
    {
		...
		else if(m_pCurLineEdit==m_pPourUWatLin)
		{
		    dlgInputPanel.SetInputMask(QString("000"));
                ...
                m_pCurLineEdit==m_pPourUSpeedA||m_pCurLineEdit==m_pPourUSpeedV)
		{
