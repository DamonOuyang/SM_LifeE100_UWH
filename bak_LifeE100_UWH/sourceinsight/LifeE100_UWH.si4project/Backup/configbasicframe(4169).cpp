...
ConfigBasicFrame::ConfigBasicFrame(QWidget *parent,QWidget *parent1) :
    QFrame(parent),
    ui(new Ui::ConfigBasicFrame)
{
    ...
    connect(this,SIGNAL(ExitLoginFlg(void)),parent1,SLOT(ExitLoginPwd(void)));
...
void ConfigBasicFrame::OnPwdModEdit(void)
{
    ...
    //pwdDlg.exec();
    PasswordDialog  pwdDlg(this,true);
	pwdDlg.setGeometry(QRect(0, 40, SET_VAL_DLG_W, SET_VAL_DLG_H));
    pwdDlg.exec(); 
}
void ConfigBasicFrame::OnExitLogin(void)
{
    ...
    /*
		...
		*/
	    ...
	    //msgBox.setGeometry(QRect(0, 40, SET_VAL_DLG_W, SET_VAL_DLG_H));
	    msgBox.SetMessLabelText("是否要退出密码验证?退出后需要重新验证!");
    ...
    {
        msgBox.SetCancelBt(false);
        msgBox.SetMessLabelText("密码还没有启用，请进入密码设置界面设置后才能启用!");
		msgBox.exec();
		//if(QMessageBox::Accepted == msgBox.exec())
		//{
		//}
