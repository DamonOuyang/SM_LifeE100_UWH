...
#endif
{
	...
	//QLabel *pMovLabel = new QLabel(this);
	ui->MovLabel->setContentsMargins(0,0,0,0);
	QMovie *pMovie = new QMovie();
	pMovie->setFileName(":/images/LogOnGIF_L0.gif");
	ui->MovLabel->setMovie(pMovie);
	pMovie->start();
	printf("-------------------------------------------------------her \r\n");
    //sleep(5);
    //pMovie->stop();
    ...
    //水浴
    connect(m_pWtrTmpSetBt, SIGNAL(clicked()),this, SLOT(on_wtrTmpSetBt_clicked(void)));
    //动脉
...
void MainWindow::FlashWaterRoomInfo(void)
{
	...
	if(m_unWatRunState == 0 && 1==ui->wtrRomOnOffBt->isChecked())
	{
...
void MainWindow::FlashPumpAInfo(void)
{
	...
	ui->prePressLb->setText(ValueToString(pumDevData.unPrssVal/10,1,0));

    //ui->pourPressLb->setText(ValueToString(pumDevData.unPrssVal,10,1));
    ui->pourPressLb->setText(ValueToString(pumDevData.unPrssVal/10,1,0));
...
{
    //ui->m_pWtrTmpSetBt->setEnabled(false);
...
{
    m_unSliderVal = 0;
	//m_pRPBar = new QRoundProgressBar(ui->WatRoundBar);
    m_pRPBar = bar;
...
void MainWindow::on_BPStrBtn_clicked(bool checked)
{
	...
	{/*
        ...
        if(QMessageBox::AcceptRole == closePumWin.exec())
		{
		    m_unPumpRunA = 0;
			m_unPourStaA = 0;
			ui->monTabWidget->setEnabled(false);
...
void MainWindow::PourPressLEClick(void)
{
	...
	if(m_pCurLineEdit == NULL)
	{
		...
		setValDialog.SetValLimit(SET_PRESS_LIMIT_MIN,g_devConfigInfo.arteryParm.unPressLimt,SET_STEP_5*2,10,1,1);
	    setValDialog.setWindowModality(Qt::ApplicationModal);
	    setValDialog.exec();
	}
	else
...
void MainWindow::PreSpeedLEClick(void)
{
    ...
    if(m_pCurLineEdit == NULL)
    {
		...
		setValDialog.SetValLimit(SET_SPEED_LIMIT_MIN,g_devConfigInfo.arteryParm.unSpeedLimt,SET_STEP_100,1,0,10);
	    setValDialog.setWindowModality(Qt::ApplicationModal);
	    setValDialog.exec();
    }
	else
...
{
    QString strPathName;
	strPathName = QString("echo %1 > /sys/class/gpio/gpio72/value").arg(checked);
	/*
...
/*
    if(unCtrlLev == ALM_LEV_H)
    {
		...
		strPathName2 = QString("echo %1 > /sys/class/gpio/gpio4/value").arg(1);
    }
    else if(unCtrlLev == ALM_LEV_M)
    {
        strPathName1 = QString("echo %1 > /sys/class/gpio/gpio23/value").arg(1);
		strPathName2 = QString("echo %1 > /sys/class/gpio/gpio4/value").arg(0);
    }
    else if(unCtrlLev == ALM_LEV_H)
    {
...
//压力上下限,流量上下限
   if(g_devAlmBits.ApumGrpAlm.bPressUBits || g_devAlmBits.ApumGrpAlm.bPressUBits ||\
	  g_devAlmBits.ApumGrpAlm.bFlowUBits || g_devAlmBits.ApumGrpAlm.bFlowLBits)
   {
...
void MainWindow::PreSpeedLEClickV(void)
{
    ...
    if(m_pCurLineEdit == NULL)
    {
		...
		setValDialog.SetValLimit(SET_SPEED_LIMIT_MIN_V,g_devConfigInfo.veinParm.unSpeedLimt,SET_STEP_100,1,0,10);
	    setValDialog.setWindowModality(Qt::ApplicationModal);
	    setValDialog.exec();
    }
	else
...
void MainWindow::FlashBattryInfo(void)
{
	...
	//batDevData.unBatSoc = 80;
	ShowChgIcon(m_unBatStat);
	ShowBatteryLinkLog(g_pWorkThread->GetBatLinkState());
	ShowACLinkLog(unDisChg);

    ui->batWidget->setValue(batDevData.unBatSoc, unDisChg);
	//ui->batSocLabel->setText(ValueToString(batDevData.unBatSoc,1,0)+"%");
