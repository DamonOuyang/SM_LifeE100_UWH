...
bool CBasicPourProc::StartRun(UINT8 unIndex)
{
    QString strPathName;
...
bool CBasicPourProc::StopRun(void)
{
    if(false==m_bRunFlag)
    {
    ...
    SaveTestData(true);
    m_bRunFlag  = false;
    //g_pourTstData.tstInfo.unStopType = MON_DATA_TYPE;
    if(NULL!=m_pWorkThread)
    {
        m_pWorkThread->OnPumStop();
    }
...
bool CBasicPourProc::IsRun(void)
{
    return m_bRunFlag;
}
