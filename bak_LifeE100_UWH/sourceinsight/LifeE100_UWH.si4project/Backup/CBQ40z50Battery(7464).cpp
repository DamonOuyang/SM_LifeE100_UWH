...
void CBq40z50Bat::DataParser(BYTE* pDataBuf,UINT32 unCmdID,UINT32 unDataLen)
{
    ...
    {
	    case CMD_TMP:
			m_unBatTmp = (UINT16)((pDataBuf[1]&0xff)<<8)+pDataBuf[0];
			//printf("----------------------------------Tmp:%u-----------------\r\n",m_unBatTmp);
		    m_unCmdFlg = 1;
		break;
	    case CMD_VOL:
			m_unBatVol = (UINT16)((pDataBuf[1]&0xff)<<8)+pDataBuf[0];
			//printf("----------------------------------VOL:%u-----------------\r\n",m_unBatVol);
		    m_unCmdFlg = 2;
		break;
	    case CMD_CUR:
			m_unBatCur = (UINT16)((pDataBuf[1]&0xff)<<8)+pDataBuf[0];
			//printf("----------------------------------Cur:%d-----------------\r\n",m_unBatCur);
		    m_unCmdFlg = 3;
		break;
	    case CMD_SOC:
			if(pDataBuf[0] < 20 || pDataBuf[0] == 0 || pDataBuf[0] > 100)
			{
			    m_unSocNum++;
				if(m_unSocNum >= 3)
				{
		...
		break; 
	    case CMD_NUM:
			m_unCycNum = (UINT16)((pDataBuf[1]&0xff)<<8)+pDataBuf[0];
			//printf("----------------------------------CycNum:%u-----------------\r\n",m_unCycNum);
		    m_unCmdFlg = 0;
			m_bGetData = true;
		break; 
