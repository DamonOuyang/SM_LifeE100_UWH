...
void CWorkThread::ProcAlmInfo(void)
{
    ...
    for(unIndex = 0; unIndex < g_devConfigInfo.basicCfgInfo.unAPumCount; unIndex++)
    {
		...
		//printf("unIndex=%d unATmpVal:%d unAlmUTmpA:%d bTmpUBits:%d m_bBpRunFlag[0]:%d m_unDataType[0]:%d \r\n",\
		       //unIndex,g_pourTstData.tstData.unATmpVal,g_devConfigInfo.devAlmParam.unAlmUTmpA,g_devAlmBits.ApumGrpAlm[unIndex].bTmpUBits,\
			   //m_bBpRunFlag[0],m_unDataType[0]);
