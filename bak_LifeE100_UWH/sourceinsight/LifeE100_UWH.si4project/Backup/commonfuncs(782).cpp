...
#define M_T_ALM_ITEM_AC_LOS        0x30    //
#define M_T_ALM_ITEM_BAT_DIS       0x31    //
#define L_T_ALM_ITEM_WAT_LIN_UP    0x32    //水浴-水位高	
...
QString AlmIDToName(UINT8 unAlmID)
{
	...
	case ALM_ERROE_A_TMP:
        strAlmName = "动脉血温异常";
        break;
	case ALM_ITEM_A_TMP_SEN:
        strAlmName = "动脉血温传感器异常";
        break;
	case ALM_ITEM_A_FLO_SEN:
        strAlmName = "动脉流量传感器异常";
        break;
	case ALM_ITEM_A_PRE_SEN:
        strAlmName = "动脉压力传感器异常";
        break;
	case ALM_ITEM_A_SPE_SEN:
        strAlmName = "动脉转速传感器异常";
        break;
//--------静脉泵----------
	...
	case ALM_ERROE_V_TMP:
        strAlmName = "静脉血温异常";
        break;
	case ALM_ITEM_V_TMP_SEN:
        strAlmName = "静脉血温传感器异常";
        break;
	case ALM_ITEM_V_FLO_SEN:
        strAlmName = "静脉流量传感器异常";
        break;
	case ALM_ITEM_V_PRE_SEN:
        strAlmName = "静脉压力传感器异常";
        break;
	case ALM_ITEM_V_SPE_SEN:
        strAlmName = "静脉转速传感器异常";
        break;	
//----------水浴-------------
	...
	case ALM_ITEM_BLOOD_LIN_DN:
	    strAlmName = "血液液位低";
	    break;	
	case ALM_ITEM_WAT_TMP_SEN:
		strAlmName = "水浴温度传感器异常";
		break;
	case ALM_ITEM_WAT_LIN_SEN:
		strAlmName = "水浴水位传感器异常";
		break;
	case ALM_ITEM_WAT_BLO_SEN:
		strAlmName = "水浴血位传感器异常";
		break;	
...
QString CheckOneAlmShow(void)
{
    ...
    for(unIndex = 0; unIndex < g_devConfigInfo.basicCfgInfo.unWatCount; unIndex++)
    {
		...
		{
			strAlmItem = QString("水浴模块<%1>---温度传感器异常").arg(unIndex+1);
			return strAlmItem;
		}
        //水浴-温度高
    ...
    for(unGrpIndex = 0; unGrpIndex < g_devConfigInfo.basicCfgInfo.unAPumCount; unGrpIndex++ )
    {
        ...
        {
            strAlmItem = QString("动脉血泵<%1>---有气泡").arg(unGrpIndex+1);
			return strAlmItem;
        }
        if(g_devAlmBits.ApumGrpAlm[unGrpIndex].bTmpUBits)
        {
            strAlmItem = QString("动脉血泵<%1>---血温高").arg(unGrpIndex+1);
			return strAlmItem;
        }
        ...
        {
            strAlmItem = QString("动脉血泵<%1>---血温异常").arg(unGrpIndex+1);
			return strAlmItem;
        }	
    ...
    {
        if(g_devAlmBits.VpumGrpAlm[unGrpIndex].bPressSensorBits)
        {
            strAlmItem = QString("静脉血泵组<%1>---压力传感器异常").arg(unGrpIndex+1);
            return strAlmItem;
        }
        //动脉泵-压力
        if(g_devAlmBits.VpumGrpAlm[unGrpIndex].bPressUBits)
        {
            strAlmItem = QString("静脉血泵<%1>---压力高").arg(unGrpIndex+1);
			return strAlmItem;
        }
        if(g_devAlmBits.VpumGrpAlm[unGrpIndex].bFlowSensorBits)
        {
            strAlmItem = QString("静脉血泵组<%1>---流量传感器异常").arg(unGrpIndex+1);
            return strAlmItem;
        }
        //动脉泵-流量
        if(g_devAlmBits.VpumGrpAlm[unGrpIndex].bFlowUBits)
        {
            strAlmItem = QString("静脉血泵<%1>---流量高").arg(unGrpIndex+1);
			return strAlmItem;
        }
        if(g_devAlmBits.VpumGrpAlm[unGrpIndex].bFlowLBits)
        {
            strAlmItem = QString("静脉血泵<%1>---流量低").arg(unGrpIndex+1);
			return strAlmItem;
        }
        if(g_devAlmBits.VpumGrpAlm[unGrpIndex].bBubblBits)
        {
            strAlmItem = QString("静脉血泵<%1>---有气泡").arg(unGrpIndex+1);
			return strAlmItem;
        }
        if(g_devAlmBits.VpumGrpAlm[unGrpIndex].bTmpSensorBits)
        {
            strAlmItem = QString("静脉血泵组<%1>---血温传感器异常").arg(unGrpIndex+1);
            return strAlmItem;
        }
        if(g_devAlmBits.VpumGrpAlm[unGrpIndex].bTmpUBits)
        {
            strAlmItem = QString("静脉血泵<%1>---血温高").arg(unGrpIndex+1);
			return strAlmItem;
        }
	    if(g_devAlmBits.VpumGrpAlm[unGrpIndex].bTmpErrBits)
        {
            strAlmItem = QString("静脉血泵<%1>---血温异常").arg(unGrpIndex+1);
			return strAlmItem;
        }
        if(g_devAlmBits.VpumGrpAlm[unGrpIndex].bSpeeSensorBits)
        {
            strAlmItem = QString("静脉血泵组<%1>---转速传感器异常").arg(unGrpIndex+1);
            return strAlmItem;
        }			
    }	
	return strAlmItem;
