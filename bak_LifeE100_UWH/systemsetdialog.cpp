...
void SystemSetDialog::UpChooseDev(unsigned char unTpye)
{
    ...
    if(!dir.isDir())
    {
		...
		msgBox.SetMessLabelText("未发现U盘，请插入U盘!");
		msgBox.exec();
    ...
    QFileInfoList list;
	QDir dir1("/udisk/UpdateFiles/");
	dir1.setFilter(QDir::Files | QDir::Dirs | QDir::NoDotAndDotDot);
	list = dir1.entryInfoList();

	//QFileInfoList list = UpdDir.entryInfoList(QDir::Files | QDir::Hidden | QDir::NoSymLinks);
	QFileInfo  UpdDir("/udisk/UpdateFiles/");
    if(!UpdDir.isDir() || list.count() <= 0)
    {
        MessageBoxDialog msgBox(this, false);
	    msgBox.setGeometry(QRect(0, 40, SET_VAL_DLG_W, SET_VAL_DLG_H));
        msgBox.SetMessLabelText("未发现升级文件，请拷入文件!");
        msgBox.exec();
		UpdateFail(0, UpdFindFileErr);
        return;
    }
    ...
    if(SystemEx(strCpy.toStdString().c_str())<0)
    {
		MessageBoxDialog msgBox(this, false);
		msgBox.setGeometry(QRect(0, 40, SET_VAL_DLG_W, SET_VAL_DLG_H));
        msgBox.SetMessLabelText("拷贝程序升级文件失败!");
        msgBox.exec();
		UpdateFail(0, UpdFindFileErr);
        return;
    }
