...
CUARTSocketBQ40Z50::CUARTSocketBQ40Z50(CSocketHandler* pSktHandler) : CUARTSocket(pSktHandler)
{
	...
	//printf("----------------------------------------CUARTSocketBQ40Z50::CCmdDataQueue(50)\r\n");
...
void CUARTSocketBQ40Z50::RecvDataFrmUART(BYTE bValue)
{
    ...
    {		
        if(0x00 == m_uartDataBuf.unDataPos)
        {
            //m_uartDataBuf.unDataPos = 0;
            //数据开始标记
            if(BAT_DEV_BAS_ID == bValue+BAT_ID_ADD)
            {
                m_uartDataBuf.pDataBuf[0] = bValue+BAT_ID_ADD;
        ...
        else if(BAT_DEV_BAS_ID == m_uartDataBuf.pDataBuf[0] && m_uartDataBuf.unDataPos < MAX_RCV_BUF_SIZE)
        {
            if(0x0A == bValue)
            {
                //数据结束标记
        ...
        {
			printf("unDataPos:%d >= 8 or BAT_DEV_BAS_ID != pDataBuf[0]:%02x-------\r\n",m_uartDataBuf.unDataPos,m_uartDataBuf.pDataBuf[0]);
			m_uartDataBuf.unDataPos   = 0;
	...
	if(m_uartDataBuf.unDataPos >= MAX_RCV_BUF_SIZE)
	{
	    printf("m_uartDataBuf.unDataPos %d-----------------------%d\r\n",m_uartDataBuf.unDataPos);
		for(UINT8 i = 0; i < m_uartDataBuf.unDataPos; i++)
		{
			printf("%02x ",m_uartDataBuf.pDataBuf[i]);
		}
		printf("\r\n");
...
void CUARTSocketBQ40Z50::OnRead()
{
    ...
    {	
		...
		//printf("--------CUARTSocketBQ40Z50::OnRead------m_RevSocket:%d-------------\r\n",m_RevSocket);
        UINT16 unFrmPos = 0;
...
void CUARTSocketBQ40Z50::SendCmdData(void)
{
	...
	m_pCmdBuf[0] = m_cmdData.unDevID-BAT_ID_ADD;
...
void CUARTSocketBQ40Z50::ProcessUARTRecvData(BYTE* pDataBuf, UINT16 unDataSize)
{
    ...
    UINT8 unDevID = pDataBuf[0];
	UINT8 unCmdID = m_unSndCmd;
